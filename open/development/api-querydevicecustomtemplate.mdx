...
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | Yes | The API credential. Obtain it by calling the [Get user token](https://open.dingtalk.io/document/development/obtain-user-token#) API.  When you call the Get user token API, the `code` parameter must be obtained from a constructed link whose scope includes the user ID and organization ID, that is, `scope=openid corpid`. |

### Path parameters

| Name | Type | Required | Description |
| --- | --- | --- | --- |
| templateId | Long | Yes | The template ID. Obtain it by calling the [Query custom screen template list](https://open.dingtalk.io/document/development/api-querydevicecustomtemplatelist#) API. |
...
If an error occurs when you call this API, look up the error message in the [Global error codes](https://open.dingtalk.io/document/development/server-api-error-codes-1) document for solutions.
