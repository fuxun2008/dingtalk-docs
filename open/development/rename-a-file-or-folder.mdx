...
| dentryId | String | Yes | The file or folder ID. Call the [Get the list of files or folders](https://open.dingtalk.io/document/development/get-a-list-of-files-or-folders) or [Get spaceId and dentryId by dentryUuid](https://open.dingtalk.io/document/development/api-getdentryidbyuuid) API to obtain the value of the `id` parameter.  The root directory of a storage space cannot be renamed, so this parameter does not support the value 0. |
...
If the API call fails, refer to the error message and look up the solution in the [Global error codes](https://open.dingtalk.io/document/development/server-api-error-codes-1) document.
