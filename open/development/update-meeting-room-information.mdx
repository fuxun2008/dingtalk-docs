...
description: "Call this API to update room information. Documents the API call method, request parameters, and response structure."
---
...
| roomId | String | Yes | The Room ID. Call the [Query the meeting room list](/open/development/check-the-meeting-room-list) API to obtain it. |
...
| groupId | Long | No | The Group ID that the Room belongs to. Call the [Query the meeting room Group list](/open/development/query-meeting-rooms-groups) API to obtain it. |
...
| memberId | String | No | The User ID, configured based on memberType:   - **User**: the User's unionId. Call the [Query user details](/open/development/query-user-details) API to obtain it. - **Department**: the Department ID (dept_id). Call the [Obtain the department list](https://open.dingtalk.com/document/development/obtain-the-department-list-v2) API to obtain the dept_id parameter value. |
...
If an error is returned when calling this API, refer to the [Global error codes](/open/development/server-api-error-codes-1) document for the solution.
