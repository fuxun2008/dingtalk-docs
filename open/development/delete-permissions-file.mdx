...
| Permission scope | permission-Storage.Permission.Write-Write permission for organization storage authorization information |
...
| Name | Type | Required | Description |
| --- | --- | --- | --- |
| dentryUuid | String | Yes | The file UUID. To obtain this value, call the [File Search](https://open.dingtalk.io/document/development/search-for-files) API or the [Get dentryUuid](https://open.dingtalk.io/document/development/api-getuuidbydentryid) API, and read the `dentryUuid` field from the response. |

### Query parameter

| Name | Type | Required | Description |
| --- | --- | --- | --- |
| unionId | String | Yes | The user unionId. |
...
| members | Array | Yes | The list of permission members. Up to 30 members are supported. |
...
| id | String | Yes | The ID of the permission member:   - When `type=ORG`, the value is the organization ID. - When `type=DEPT`, the value is the department ID. - When `type=TAG`, the value is the tag ID. - When `type=CONVERSATION`, the value is the chat ID. - When `type=USER`, the value is the employee userId. |
| corpId | String | No | The organization to which the permission belongs.       - If an organization ID is provided, the permission is automatically removed when the corresponding member leaves the organization. - If `memberType` is `dept`, the organization ID is required. |
...
If an error is returned when you call this API, refer to the [Global error codes](https://open.dingtalk.io/document/development/server-api-error-codes-1) documentation for solutions.
...
| 403 | permissionDenied | %s | The user lacks the required permission. |
