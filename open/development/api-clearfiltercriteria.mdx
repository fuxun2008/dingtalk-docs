...
| Name | Type | Required | Description |
| --- | --- | --- | --- |
| operatorId | String | Yes | The unionId of the operator.   - You can call the [Get user information by silent login code](https://open.dingtalk.io/document/orgapp/obtain-the-userid-of-a-user-by-using-the-log-free) API to obtain the value of the unionid parameter. - You can call the [Query user details](https://open.dingtalk.io/document/orgapp/query-user-details) API to obtain the value of the unionid parameter.       If the operator does not have permission, the API returns the error `The operator has no permission`. |
