---
title: "Get filter"
description: "Call this API to retrieve the currently active global filter on a specified worksheet, returning the filter ID, range, and per-column conditions, suitable for confirming filter state or reading configuration."
---

Call this API to query the global filter currently in effect on a specified worksheet. The response returns the filter ID, the filter range, and the filter conditions set for each column. This is useful for confirming the current filter state before modifying filter conditions, or for reading the filter configuration programmatically for further processing.
...
| Name | Type | Required | Description |
| --- | --- | --- | --- |
| workbookId | String | Yes | The ID of the spreadsheet file. The `nodeId(dentryUuid)` returned by the [Knowledge Base API](/open/development/knowledge-base-overview) is the `workbookId` of the spreadsheet. Call the [Get node](/open/development/get-knowledge-base-acquisition-node) or [Create Knowledge Base document](/open/development/create-team-space-document) API to obtain it. |
| sheetId | String | Yes | The ID or name of the worksheet. Call the [Get all worksheets](/open/development/obtain-all-worksheets) API to obtain the `id` or `name` parameter value.  **Note**  If no filter exists on the worksheet, the API returns a 404 error. |
...
If an error occurs when calling this API, look up the error message in the [Global error codes](/open/development/server-api-error-codes-1) documentation to find a solution.
