...
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | Yes | The access credential for calling this API. Obtain it as follows:   - For an internal app, call the [Obtain the access token of an internal app](https://open.dingtalk.io/document/development/obtain-the-access-token-of-an-internal-app#) API. - For a third-party enterprise app, call the [Obtain the access token of an enterprise that authorizes a third-party app](https://open.dingtalk.io/document/development/obtain-the-access-token-of-the-authorized-enterprise-1#) API. - For a third-party personal app, call the [Obtain user token](https://open.dingtalk.io/document/development/obtain-user-token#) API. |

### Path parameters

| Name | Type | Required | Description |
| --- | --- | --- | --- |
| userId | String | Yes | The unionId of the user to query.   - For internal apps and third-party enterprise apps, call the [Query user details](https://open.dingtalk.io/document/development/query-user-details#) API to obtain the `unionid` parameter value. - For third-party personal apps, call the [Obtain a user's profile from Contacts](https://open.dingtalk.io/document/development/dingtalk-retrieve-user-information#) API to obtain the `unionId` parameter value. |
| calendarId | String | Yes | The calendar ID to which the event belongs. Call the [Query calendars](https://open.dingtalk.io/document/development/query-a-calendar#) API to query events for the current user. Use the keyword **primary** to refer to the primary calendar of the current user. |
...
| maxAttendees | Integer | No | The number of attendees returned for each event. The default value is 100, and the maximum value is 100. If the number of attendees exceeds 100 and you need to retrieve all attendees, use the [Get event attendees](https://open.dingtalk.io/document/development/get-the-participants-of-a-schedule#) API. |
