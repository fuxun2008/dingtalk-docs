...
| parentId | String | No | The parent folder ID. For the root folder, this parameter is `0`. Call the [Get the file or folder list](https://open.dingtalk.io/document/development/obtain-the-file-list-storage#) API to obtain the value of the `parentId` parameter. Used for file name conflict checks within the same folder. |
...
If an error is returned when you call this API, search for the error message in the [Global error codes](https://open.dingtalk.io/document/development/server-api-error-codes-1) document for a solution.
