...
| Name | Type | Required | Description |
| --- | --- | --- | --- |
| operatorId | String | Yes | The unionId of the operator.   - Call the [Obtain user information by using a silent login code](https://open.dingtalk.io/document/orgapp/obtain-the-userid-of-a-user-by-using-the-log-free) API to obtain the unionid parameter value. - Call the [Query user details](https://open.dingtalk.io/document/orgapp/query-user-details) API to obtain the unionid parameter value.       If the operator has no permission, the API returns the error `The operator has no permission`. |
...
If the API call returns an error, look up the solution in the [Global error codes](https://open.dingtalk.io/document/development/server-api-error-codes-1) documentation based on the error message.
