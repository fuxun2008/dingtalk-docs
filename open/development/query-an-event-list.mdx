---
title: "List events"
description: "Call this API to query the event list of a user within a specified time range, including event title, start date, start time, end date, and end time."
---
...
- By default, deleted events are not returned. To return them, set `showDeleted=true`. When a deleted event is returned, calling the [Get event details](/open/development/query-details-about-an-event#) API returns the **instanceNotExist** error.
...
| maxAttendees | Integer | No | The number of attendees to query per event. Default value: 100. Maximum value: 100. If an event has more than 100 attendees and you need to retrieve all of them, use the [Get event attendees](/open/development/get-the-participants-of-a-schedule#) API. |
