...
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | Yes | The API credential. Call the [Get user token](https://open.dingtalk.io/document/development/obtain-user-token#) API to obtain it. When constructing the link to obtain the `code` parameter used in the request to get the user token, the `scope` must include both the user ID and the organization ID, that is, `scope=openid corpid`. |
...
| logo | String | No | The template logo. Call the [Upload media file](https://open.dingtalk.io/document/development/upload-media-files#) API to obtain the mediaId after uploading the image. Image size: 240\*240. The size of a single image must not exceed 1 MB. |
...
| bgImgList | Array of String | No | The image mediaId. Call the [Upload media file](https://open.dingtalk.io/document/development/upload-media-files#) API to obtain it. You can upload up to 9 images, and the size of each image must not exceed 5 MB. |
...
| deviceUnionIds | Array of String | No | The device unionId. Obtain this parameter by subscribing to the [Device-room binding change](https://open.dingtalk.io/document/development/org-event-overview#) event. |
| groupIds | Array of Long | No | The group ID. Call the [Query meeting room group list](https://open.dingtalk.io/document/development/query-meeting-rooms-groups#) API to obtain it. |
| roomIds | Array of String | No | The room ID. Call the [Query meeting room list](https://open.dingtalk.io/document/development/check-the-meeting-room-list#) API to obtain it. |
