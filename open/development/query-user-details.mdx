...
| name | String | 张三 | The Employee's Name. |
...
| mobile | String | 18513027676 | The phone number.  **Note**   - For an Internal app, this field is returned only when the App has been granted the **Employee phone number** permission in Contacts. - This field is not returned for a Third-party enterprise app. To obtain mobile, use the DingTalk unified authorization suite. |
...
| extension | String | `{"爱好":"旅游","年龄":"24"}` | Extended Attributes, up to 2000 characters in length.  **Note**   - Returned only when an extended Field added on the Employee Profile panel has a value. - For an Internal app, this field is returned only when the App has been granted the **Mail and other Profile information** permission in Contacts. - This field is not returned for a Third-party enterprise app. - **Important**: Adding a new Attribute directly overwrites existing Attribute values. First fetch the existing Attributes, append the new Attribute, and then perform a full Update. |
...
| extension_i18n | Json | `{"爱好": {"zh_CN": "旅游", "en_US": "travel", "aJP": "旅行"}}` | Internationalized values of the extended Attributes. |
