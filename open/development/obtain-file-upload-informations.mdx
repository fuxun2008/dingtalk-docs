...
- If the `spaceId` parameter is provided when uploading a file, use the [Get file upload information](https://open.dingtalk.io/document/development/obtain-storage-upload-information#) API.
...
| Name | Type | Required | Description |
| --- | --- | --- | --- |
| parentDentryUuid | String | Yes | The dentryUuid of the parent node. Call the [Search for files](https://open.dingtalk.io/document/development/search-for-files#) or [Get dentryUuid information](https://open.dingtalk.io/document/development/api-getuuidbydentryid#) API to obtain the value of the returned `dentryUuid` field.    For the space root directory, pass the dentryUuid of the space root directory. |
