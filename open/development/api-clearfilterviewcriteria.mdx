---
title: "Clear filter view criteria"
---

Call this API to clear the filter criteria of a specific column in a filter view, so that the column no longer participates in filter calculations. This applies when you want to remove the filter rules of one column in a personal filter view while keeping the filter state of other columns.

## API call description

The clear operation only affects the current filter view and does not affect global filters or other filter views.
...
| sheetId | String | Yes | The worksheet ID or name. Obtain the id or name parameter value by calling the [Get all worksheets](https://open.dingtalk.io/document/orgapp/obtain-all-worksheets) API. |
...
| Name | Type | Required | Description |
| --- | --- | --- | --- |
| operatorId | String | Yes | The unionId of the operator.   - Obtain the unionid parameter value by calling the [Get user information by silent login code](https://open.dingtalk.io/document/orgapp/obtain-the-userid-of-a-user-by-using-the-log-free) API. - Obtain the unionid parameter value by calling the [Query user details](https://open.dingtalk.io/document/orgapp/query-user-details) API.       If the operator does not have permission, the API returns the error `The operator has no permission`. |
