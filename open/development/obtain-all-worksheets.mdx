...
| Name | Type | Required | Description |
| --- | --- | --- | --- |
| operatorId | String | Yes | The unionId of the operator.   - Obtain the unionid parameter value by calling the [Get user information by silent login code](https://open.dingtalk.io/document/development/obtain-the-userid-of-a-user-by-using-the-log-free#) API. - Obtain the unionid parameter value by calling the [Query user details](https://open.dingtalk.io/document/development/query-user-details#) API.     If the operator does not have the required permissions, the API returns the error `The operator has no permission`. |
