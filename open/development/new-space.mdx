...
Call this API to create a shared space for your organization.
...
| Required permissions | permission-Drive.Space.Write-Write permission on Drive app space |
...
| Name | Type | Required | Description |
| --- | --- | --- | --- |
| name | String | Yes | The space name. Cannot be empty. |
| unionId | String | Yes | The unionId of the user. To obtain it, call the [Query user details](https://open.dingtalk.io/document/development/query-user-details#) API. |
...
| spaceType | String | The space type. |
| quota | Long | The total space quota. |
| usedQuota | Long | The used space quota. |
...
If the API call fails, refer to the [Global error codes](https://open.dingtalk.io/document/development/server-api-error-codes-1) document and locate a solution based on the error message.
...
| 400 | no.priviledge | You are not authorized to perform this operation. | You are not authorized to perform this operation. |
| 400 | addSpace.forbiden | Add space is forbiden. | The admin of your organization has disabled creating new shared spaces. Existing spaces can still be used. |
| 400 | name.invalid | Space name is invalid. | The space name is invalid. |
