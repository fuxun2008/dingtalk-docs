...
This API supports internal app bots. For more information, see [Create an internal app bot](https://open.dingtalk.io/document/dingstart/configure-the-robot-application#).
...
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | Yes | The access credential used to call this API. Obtain it as follows:   - For internal apps, call the [Get the access token of an internal app](https://open.dingtalk.io/document/development/obtain-the-access-token-of-an-internal-app#) API. - For third-party enterprise apps, call the [Get the access token of an authorized organization for a third-party app](https://open.dingtalk.io/document/development/obtain-the-access-token-of-the-authorized-enterprise-1#) API. |

### Request body

| Name | Type | Required | Description |
| --- | --- | --- | --- |
| msgParam | String | Yes | The message template parameters. For more information, see [Message types for enterprise bots](https://open.dingtalk.io/document/orgapp/types-of-messages-sent-by-robots#). The length must not exceed 15,000 bytes. |
| msgKey | String | Yes | The message template key. For more information, see [Message types for enterprise bots](https://open.dingtalk.io/document/dingstart/types-of-messages-sent-by-robots#). |
| openConversationId | String | Yes | The chat ID:   - For an internal group    - To obtain the ID of a newly created internal group, internal apps can call the [Create an internal group](https://open.dingtalk.io/document/development/create-group-session#) API. To obtain the ID of an existing internal group, call [chooseChat](https://open.dingtalk.io/document/development/jsapi-choose-chat#). - For a scenario group    - Internal apps can call the [Create a scenario group](https://open.dingtalk.io/document/development/create-a-scene-group-v2#) API. To obtain the ID of an existing scenario group, use the [chooseChat](https://open.dingtalk.io/document/orgapp/jsapi-choose-chat#) JSAPI. |
| robotCode | String | Yes | The bot code. For more information, see [Bot ID](https://open.dingtalk.io/document/development/development-robot-overview#). |
| coolAppCode | String | No | The group chat cool app code. For more information, see [Group chat cool app](https://open.dingtalk.io/document/dingstart/configuration-group-chat-quick-entry#).    This parameter is **required** when the bot is installed through a group chat cool app. |
...
| --- | --- | --- |
| processQueryKey | String | The encrypted message ID. Use this ID to query the read status of the message and recall the message by calling the [Query the read list of bot messages in person-to-person chats](https://open.dingtalk.io/document/development/query-the-read-list-of-robot-messages-in-person-to-person-conversations#) and [Recall internal group messages sent by an enterprise bot](https://open.dingtalk.io/document/development/enterprise-chatbot-withdraws-internal-group-messages#) APIs. |
...
If an error occurs when you call this API, find the solution in the [Global error codes](https://open.dingtalk.io/document/development/server-api-error-codes-1) topic based on the error message.
