...
| Name | Type | Required | Description |
| --- | --- | --- | --- |
| operatorId | String | Yes | The unionId of the operator.   - Call the [Obtain the user ID of a user by using a silent login code](https://open.dingtalk.io/document/orgapp/obtain-the-userid-of-a-user-by-using-the-log-free) API to obtain the value of the unionid parameter. - Call the [Query user details](https://open.dingtalk.io/document/orgapp/query-user-details) API to obtain the value of the unionid parameter.       If the operator has no permission, the API returns the error `The operator has no permission`. |
