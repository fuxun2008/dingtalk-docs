---
title: "Kosongkan data dalam julat sel"
description: "Panggil API ini untuk mengosongkan data dalam julat sel sambil mengekalkan pemformatan sedia ada."
---

Panggil API ini untuk mengosongkan data dalam julat sel, tidak termasuk pemformatan.
...
| Medan | Nilai |
| --- | --- |
| URL HTTP | `https://api.dingtalk.io/v1.0/doc/workbooks/{workbookId}/sheets/{sheetId}/ranges/{rangeAddress}/clearData` |
| Kaedah HTTP | POST |
...
| sheetId | String | Ya | ID atau Tajuk Helaian Kerja.   - Untuk aplikasi dalaman, panggil API [Dapatkan semua Helaian Kerja](/ms/open/development/obtain-all-worksheets#) untuk mendapatkan ID, atau berikan Tajuk Helaian Kerja. - Untuk aplikasi perusahaan pihak ketiga, berikan Tajuk Helaian Kerja.   Lihat rajah di bawah untuk cara melihat Tajuk Helaian Kerja. |
| rangeAddress | String | Ya | Julat sel yang hendak dikosongkan, dalam format `sel kiri atas julat:sel kanan bawah julat`.  Sebagai contoh, B2:C3 mewakili julat yang ditunjukkan dalam rajah di bawah. |
...
| 403 | forbidden.acrossOrg | %s | Permintaan tidak sah. Sahkan bahawa dokumen yang hendak diakses adalah Milik Organisasi yang dinyatakan oleh access token. |
| 403 | forbidden.operationIllegal | %s | Tindakan permintaan tidak sah. Semak Mesej ralat. |
| 403 | forbidden.document.sizeOverLimit | The document size is over limit and the server is unable to complete your request. Retry is unlikely to work unless the document size is decreased. | Kandungan Buku Kerja terlalu besar. Cuba kurangkannya. |
| 404 | invalidRequest.resource.notFound | %s | Permintaan gagal. Sumber yang hendak diakses tidak ditemui. |
