---
title: "Dapatkan maklumat sibuk/lapang pengguna"
description: "Panggil API ini untuk menyemak status sibuk/lapang senarai pengguna dalam julat masa tertentu, sesuai untuk penjadualan syif dan tempahan mesyuarat DingTalk."
---

Panggil API ini untuk menyemak maklumat sibuk/lapang senarai pengguna yang ditentukan dalam julat masa tertentu.
...
| Keperluan kebenaran | permission-Calendar.EventSchedule.Read-Kebenaran Telah dibaca untuk status sibuk/lapang acara dalam aplikasi Kalendar |

### Header permintaan

| Nama | Jenis | Wajib | Penerangan |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | Ya | Kredensial akses untuk memanggil API ini. Dapatkan seperti berikut:   - Untuk aplikasi dalaman, panggil API [Get the access token of an internal app](/open/development/obtain-the-access-token-of-an-internal-app#). - Untuk aplikasi perusahaan pihak ketiga, panggil API [Get the access token of an enterprise that authorized a third-party app](https://open.dingtalk.com/document/development/obtain-the-access-token-of-the-authorized-enterprise-1#). - Untuk aplikasi peribadi pihak ketiga, panggil API [Get user token](/open/development/obtain-user-token#). |

### Parameter laluan

| Nama | Jenis | Wajib | Penerangan |
| --- | --- | --- | --- |
| userId | String | Ya | unionId pengguna yang memiliki sumber bagi permintaan ini.   - Untuk aplikasi dalaman dan aplikasi perusahaan pihak ketiga, panggil API [Query user details](/open/development/query-user-details#) untuk mendapatkan nilai parameter unionid. - Untuk aplikasi peribadi pihak ketiga, panggil API [Get user contacts profile](/open/development/dingtalk-retrieve-user-information#) untuk mendapatkan nilai parameter unionId. |

### Isi permintaan

| Nama | Jenis | Wajib | Penerangan |
| --- | --- | --- | --- |
| userIds | Array of String | Ya | unionId pengguna sasaran yang hendak disemak.   - Untuk aplikasi dalaman dan aplikasi perusahaan pihak ketiga, panggil API [Query user details](/open/development/query-user-details#) untuk mendapatkan nilai parameter unionid. - Untuk aplikasi peribadi pihak ketiga, panggil API [Get user contacts profile](/open/development/dingtalk-retrieve-user-information#) untuk mendapatkan nilai parameter unionId.       Panjang maksimum senarai pengguna ialah 20. |
| startTime | String | Ya | Masa mula bagi carian. |
| endTime | String | Ya | Masa tamat bagi carian. |
...
| --- | --- | --- |
| scheduleInformation | Array | Maklumat sibuk/lapang pengguna. |
| userId | String | unionId pengguna sasaran yang disemak. |
| error | String | Mesej ralat. |
| scheduleItems | Array | Senarai maklumat sibuk/lapang. |
...
| dateTime | String | Masa mula acara, dalam format `date-time` mengikut `ISO-8601`. |
| timeZone | String | Nama zon waktu, mengikut standard **IANA Time Zone Database**, contohnya `Asia/Shanghai`.      Zon waktu untuk acara sepanjang hari tetap menggunakan UTC. Untuk acara bukan sepanjang hari, zon waktu lalai ialah `Asia/Shanghai`. |
| end | Object | Masa tamat acara. |
| date | String | Tarikh tamat acara, dalam format `yyyy-MM-dd`. |
| dateTime | String | Masa tamat acara, dalam format `date-time` mengikut `ISO-8601`. |
| timeZone | String | Nama zon waktu, mengikut standard **IANA Time Zone Database**, contohnya `Asia/Shanghai`.      Zon waktu untuk acara sepanjang hari tetap menggunakan UTC. Untuk acara bukan sepanjang hari, zon waktu lalai ialah `Asia/Shanghai`. |
...
Jika ralat dikembalikan semasa memanggil API ini, rujuk dokumen [Global error codes](/open/development/server-api-error-codes-1) untuk penyelesaian berdasarkan mesej ralat.
