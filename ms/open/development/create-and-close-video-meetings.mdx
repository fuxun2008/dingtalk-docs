...
Prasyarat: Lengkapkan proses [cipta dan konfigurasi aplikasi](/open/dingstart/create-application).
...
Langkah 3: Dapatkan kredensial akses aplikasi dengan mengikuti [Dapatkan access token bagi aplikasi dalaman](/open/development/obtain-the-access-token-of-an-internal-app). Access token digunakan untuk mengesahkan pemanggil apabila memanggil API.

Langkah 4: Panggil Server API berkaitan mesyuarat.

1. Panggil Server API [Cipta mesyuarat video](/open/development/create-a-video-conference) untuk mencipta mesyuarat video dan mendapatkan `conferenceId` mesyuarat.
2. Berdasarkan `conferenceId`, panggil Server API [Pertanyaan kelompok maklumat mesyuarat video](/open/development/batch-query-of-video-conference-information) untuk mendapatkan maklumat mesyuarat dan senarai ahli.
3. Berdasarkan `conferenceId`, panggil Server API [Mulakan rakaman awan untuk mesyuarat video](/open/development/video-conference-open-cloud-recording) untuk memulakan rakaman awan.
4. Berdasarkan `conferenceId`, panggil Server API [Mulakan rakaman awan untuk mesyuarat video](/open/development/video-conference-open-cloud-recording) untuk menghentikan rakaman awan.
5. Berdasarkan `conferenceId`, panggil Server API [Tutup mesyuarat video](/open/development/close-audio-video-conference) untuk menutup mesyuarat.
6. Pertanyaan maklumat rakaman mesyuarat.

   1. Berdasarkan `conferenceId`, panggil Server API [Pertanyaan butiran rakaman mesyuarat](/open/development/query-recording-information) untuk mendapatkan maklumat seperti perakam, masa mula rakaman, dan tempoh rakaman.
   2. Berdasarkan `conferenceId`, panggil Server API [Pertanyaan maklumat video rakaman mesyuarat](/open/development/queries-the-playback-information-about-a-recorded-cloud-video) untuk mendapatkan maklumat seperti URL muat turun video, saiz fail video, dan tempoh video.
   3. Berdasarkan `conferenceId`, panggil Server API [Pertanyaan maklumat teks rakaman mesyuarat](/open/development/queries-the-text-information-about-cloud-recording) untuk mendapatkan maklumat seperti kandungan teks rakaman, masa mula rekod teks, dan masa tamat rekod teks.

## Prasyarat

Lengkapkan proses [cipta dan konfigurasi aplikasi](/open/dingstart/create-application).
...
Menggunakan client ID dan Client Secret daripada Langkah 1, dapatkan access token aplikasi dengan mengikuti [Dapatkan access token bagi aplikasi dalaman](/open/development/obtain-the-access-token-of-an-internal-app).
...
1. Panggil Server API [Cipta mesyuarat video](/open/development/create-a-video-conference) untuk mencipta mesyuarat video dan mendapatkan `conferenceId`.
...
3. Berdasarkan `conferenceId`, panggil Server API [Mulakan rakaman awan untuk mesyuarat video](/open/development/video-conference-open-cloud-recording) untuk memulakan rakaman awan.
   ...
   ```
4. Berdasarkan `conferenceId`, panggil Server API [Hentikan rakaman awan untuk mesyuarat video](/open/development/video-conferencing-stops-cloud-recording) untuk menghentikan rakaman awan.
