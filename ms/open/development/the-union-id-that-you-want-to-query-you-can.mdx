...
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | Yes | Kredensial akses untuk memanggil API ini. Perolehnya seperti berikut:   - Untuk aplikasi dalaman, panggil API [Obtain the access token of an internal app](/open/development/obtain-the-access-token-of-an-internal-app#). - Untuk aplikasi perusahaan pihak ketiga, panggil API [Obtain the access token of an enterprise authorized to a third-party app](https://open.dingtalk.com/document/development/obtain-the-access-token-of-the-authorized-enterprise-1#). |

### Query parameters

| Name | Type | Required | Description |
| --- | --- | --- | --- |
| unionId | String | Yes | unionId bagi akaun standard asal. Untuk memperoleh nilai parameter unionid, panggil API [Obtain user information by using a silent login code](/open/development/obtain-the-userid-of-a-user-by-using-the-log-free#). |
...
Jika ralat dikembalikan semasa anda memanggil API ini, cari penyelesaiannya dengan menyemak mesej ralat dalam dokumen [Global error codes](/open/development/server-api-error-codes-1).
