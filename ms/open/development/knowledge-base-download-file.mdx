---
title: "Muat turun fail dari Knowledge Base"
description: "Panggil API ini untuk mendapatkan maklumat muat turun bagi fail dalam Knowledge Base, berserta butiran permintaan dan respons."
---

Panggil API ini untuk mendapatkan maklumat muat turun bagi fail dalam Knowledge Base.

## Penerangan panggilan API

Untuk mendapatkan maklumat muat turun, ikuti langkah berikut:

Langkah 1: Panggil API ini untuk mendapatkan maklumat muat turun fail, termasuk URL muat turun dan pengepala.

Langkah 2: Lakukan muat turun. Lihat contoh berikut:

```java
public static void main(String[] args) {
  String url = ""; // The internalResourceUrls obtained from the download info API.
  String path = "/Users/xxxxx/Downloads/test.txt"; // The target path for the file download.
  // The headers information obtained from the download info API.
  Map<String, String> headers = "" // The headers obtained from the download info API.
...
<Note>
Jika fail terlalu besar dan anda perlu mendapatkan sumber secara berperingkat, tambah pengepala berikut dalam contoh di atas: `headers.put("Range","bytes=0-499");`. Untuk butiran, lihat [Muat turun sebahagian kandungan dengan menentukan Range](/ms/open/development/use-range-download-a-part-of-content).
</Note>
...
| Kebenaran yang diperlukan | permission-Storage.DownloadInfo.Read-Kebenaran Telah dibaca untuk maklumat muat turun fail storan organisasi |
...
| Nama | Jenis | Wajib | Penerangan |
| --- | --- | --- | --- |
| spaceId | String | Ya | ID ruang. Panggil API [Dapatkan senarai nod](/ms/open/development/get-node-list) untuk mendapatkan `dentryUuid` bagi nod Knowledge Base, kemudian panggil API [Dapatkan spaceId melalui dentryUuid](/ms/open/development/api-getdentryidbyuuid) untuk mendapatkan `spaceId`. |
| dentryId | String | Ya | ID fail. Panggil API [Dapatkan senarai nod](/ms/open/development/get-node-list) untuk mendapatkan `dentryUuid` bagi nod Knowledge Base, kemudian panggil API [Dapatkan spaceId melalui dentryUuid](/ms/open/development/api-getdentryidbyuuid) untuk mendapatkan `dentryId`. |

### Parameter pertanyaan

| Nama | Jenis | Wajib | Penerangan |
| --- | --- | --- | --- |
| unionId | String | Ya | unionId bagi pengendali. Dapatkan dengan memanggil API [Pertanyaan butiran pengguna](/ms/open/development/query-user-details). |
...
| version | Long | Tidak | Versi Fail. Dapatkan dengan memanggil API [Dapatkan senarai versi fail](/ms/open/development/obtains-a-list-of-file-versions). |
| preferIntranet | Boolean | Tidak | Sama ada mengutamakan penghantaran intranet. Untuk menggunakan parameter ini, penghantaran intranet storan khusus mesti Dikonfigurasi.   - **true** (Lalai): Ya - **false**: Tidak |
...
| --- | --- | --- |
| protocol | String | Protokol muat turun fail.   - **HEADER_SIGNATURE**: Tandatangani permintaan header |
| headerSignatureInfo | Object | Maklumat tandatangani permintaan header.  **Nota**  Medan ini berkuat kuasa apabila nilai medan protocol ialah HEADER_SIGNATURE. |
| resourceUrls | Array of String | Beberapa URL muat turun. URL yang disenaraikan lebih awal mempunyai keutamaan lebih tinggi. |
...
| internalResourceUrls | Array of String | URL intranet.  **Nota**  Medan ini pada masa ini tidak digunakan. Abaikan sahaja. |
...
Jika ralat dikembalikan semasa memanggil API ini, dapatkan penyelesaian berdasarkan Mesej ralat dalam dokumen [Kod ralat global](/ms/open/development/server-api-error-codes-1).
...
| 400 | fileArchived | %s | Fail berada dalam keadaan Diarkibkan. Nyahkibkan dicetuskan secara automatik. Cuba Lagi kemudian. |
