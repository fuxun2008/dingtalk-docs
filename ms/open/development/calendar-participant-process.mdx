...
Prasyarat: Lengkapkan aliran [Penciptaan dan konfigurasi aplikasi](/open/dingstart/create-application).
...
Langkah 3: Dapatkan kredensial akses Aplikasi dengan mengikuti [Dapatkan access token aplikasi dalaman](/open/development/obtain-the-access-token-of-an-internal-app). Access token digunakan untuk mengesahkan identiti pemanggil semasa memanggil API.

Langkah 4: Panggil API berkaitan acara di bahagian pelayan.

1. Panggil Server API [Acara Baharu](/open/development/create-schedule) untuk mencipta acara dan mendapatkan semula `id` acara.
2. Berdasarkan `id` acara, panggil Server API [Tambah peserta acara](/open/development/add-schedule-participant) untuk menambah peserta acara.
3. Berdasarkan `id` acara, panggil Server API [Dapatkan peserta acara](/open/development/get-the-participants-of-a-schedule) untuk mendapatkan semula maklumat peserta.
4. Berdasarkan `id` acara, panggil Server API [Tetapkan status respons jemputan acara](/open/development/configure-response-status) untuk menetapkan status respons jemputan peserta.
5. Berdasarkan `id` acara, panggil Server API [Padam Peserta Acara](/open/development/delete-schedule-participant) untuk mengeluarkan peserta acara.

## Prasyarat

Lengkapkan aliran [Penciptaan dan konfigurasi aplikasi](/open/dingstart/create-application).
