...
description: "Panggil API ini untuk memberikan izin akses kepada anggota file di DingTalk berdasarkan parameter seperti uuid file, unionId operator, dan ID izin — bagi admin yang mengelola berbagi file."
---

Panggil API ini untuk menambahkan izin bagi anggota izin berdasarkan UUID file, unionId operator, ID peran, dan informasi lainnya.
...
| Izin yang diwajibkan | permission-Storage.Permission.Write-Izin tulis untuk informasi otorisasi penyimpanan organisasi |
...
| Nama | Tipe | Wajib | Deskripsi |
| --- | --- | --- | --- |
| dentryUuid | String | Ya | UUID file. Panggil API [Pencarian File](/id/open/development/search-for-files) atau API [Mendapatkan informasi dentryUuid](/id/open/development/api-getuuidbydentryid) untuk memperoleh nilai parameter respons `dentryUuid`. |

### Parameter query

| Nama | Tipe | Wajib | Deskripsi |
| --- | --- | --- | --- |
| unionId | String | Ya | unionId pengguna. Panggil API [Kueri detail pengguna](/id/open/development/query-user-details) untuk memperoleh nilai parameter unionid. |

### Isi permintaan

| Nama | Tipe | Wajib | Deskripsi |
| --- | --- | --- | --- |
| roleId | String | Ya | ID peran. Nilai enum:   - **OWNER**: Pemilik, dengan cakupan izin berikut:    - PermissionPrivilegeEnum.INFO   - PermissionPrivilegeEnum.LIST   - PermissionPrivilegeEnum.PREVIEW   - PermissionPrivilegeEnum.READ   - PermissionPrivilegeEnum.WRITE   - PermissionPrivilegeEnum.DOWNLOAD   - PermissionPrivilegeEnum.ADD   - PermissionPrivilegeEnum.DELETE   - PermissionPrivilegeEnum.MODIFY   - PermissionPrivilegeEnum.COPY   - PermissionPrivilegeEnum.RENAME   - PermissionPrivilegeEnum.READ_PERMISSION   - PermissionPrivilegeEnum.WRITE_PERMISSION   - PermissionPrivilegeEnum.ASSIGN - **MANAGER**: Manajer, dengan cakupan izin berikut:    - PermissionPrivilegeEnum.INFO   - PermissionPrivilegeEnum.LIST   - PermissionPrivilegeEnum.PREVIEW   - PermissionPrivilegeEnum.READ   - PermissionPrivilegeEnum.WRITE   - PermissionPrivilegeEnum.DOWNLOAD   - PermissionPrivilegeEnum.ADD   - PermissionPrivilegeEnum.DELETE   - PermissionPrivilegeEnum.MODIFY   - PermissionPrivilegeEnum.COPY   - PermissionPrivilegeEnum.RENAME   - PermissionPrivilegeEnum.READ_PERMISSION   - PermissionPrivilegeEnum.WRITE_PERMISSION - **EDITOR**: Editor, dengan cakupan izin berikut:    - PermissionPrivilegeEnum.INFO   - PermissionPrivilegeEnum.LIST   - PermissionPrivilegeEnum.PREVIEW   - PermissionPrivilegeEnum.READ   - PermissionPrivilegeEnum.WRITE   - PermissionPrivilegeEnum.DOWNLOAD   - PermissionPrivilegeEnum.ADD - **DOWNLOADER**: Penampil dengan izin unduh, dengan cakupan izin berikut:    - PermissionPrivilegeEnum.INFO   - PermissionPrivilegeEnum.LIST   - PermissionPrivilegeEnum.PREVIEW   - PermissionPrivilegeEnum.READ   - PermissionPrivilegeEnum.DOWNLOAD - **READER**: Hanya tampilan, dengan cakupan izin berikut:    - PermissionPrivilegeEnum.INFO   - PermissionPrivilegeEnum.LIST   - PermissionPrivilegeEnum.PREVIEW |
| members | Array | Ya | Daftar anggota izin. Maksimal 30 anggota diperbolehkan. |
| type | String | Ya | Tipe anggota izin. Nilai enum:   - **ORG**: Organisasi - **DEPT**: Departemen - **TAG**: Tag kustom - **CONVERSATION**: Obrolan - **USER**: Pengguna |
| id | String | Ya | ID anggota izin:   - Jika `type=ORG`, id adalah ID organisasi. - Jika `type=DEPT`, id adalah ID departemen. - Jika `type=TAG`, id adalah ID tag. - Jika `type=CONVERSATION`, id adalah ID obrolan. - Jika `type=USER`, id adalah userId karyawan. |
| corpId | String | Tidak | Organisasi tempat izin berada:       - Jika ID organisasi ada, izin secara otomatis dihapus saat anggota terkait meninggalkan organisasi. - Jika memberType adalah dept, ID organisasi wajib diisi. |
| name | String | Tidak | Nama pengguna:   - Jika `type=ORG`, name adalah nama organisasi. - Jika `type=DEPT`, name adalah nama departemen. - Jika `type=TAG`, name adalah nama tag. - Jika `type=CONVERSATION`, name adalah nama obrolan. - Jika `type=USER`, name adalah nama karyawan. |
    ...
    public static void main(String[] args_) throws Exception {
        ...
        try {
            ...
            if (!com.aliyun.teautil.Common.empty(err.code) && !com.aliyun.teautil.Common.empty(err.message)) {
                // err berisi properti code dan message, yang membantu Anda menemukan issue
            }
...
Jika error dikembalikan saat Anda memanggil API ini, cari solusinya dalam dokumen [Kode error global](/id/open/development/server-api-error-codes-1) berdasarkan pesan error.
