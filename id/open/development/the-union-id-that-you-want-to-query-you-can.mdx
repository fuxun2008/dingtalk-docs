---
title: "Kueri unionId hasil migrasi berdasarkan unionId asli"
description: "Panggil API ini untuk mengueri unionId hasil migrasi berdasarkan unionId asli setelah akun standar dimigrasikan ke Akun Perusahaan di DingTalk."
---

# Kueri unionId hasil migrasi berdasarkan unionId asli
...
| Bidang | Nilai |
| --- | --- |
| URL HTTP | https://api.dingtalk.io/v1.0/contact/orgAccount/getMigrationUnionIdByUnionIds |
| Metode HTTP | GET |
| Tipe aplikasi yang didukung | appType-Aplikasi internal　appType-Aplikasi perusahaan pihak ketiga |
| Izin yang wajib | permission-Contact.OrgAccountMigration.Read-Izin baca untuk data migrasi Akun Perusahaan Kontak |
...
| Nama | Tipe | Wajib | Deskripsi |
| --- | --- | --- | --- |
| unionId | String | Ya | unionId dari akun standar asli. Untuk mendapatkan nilai parameter unionid, panggil API [Dapatkan informasi pengguna menggunakan kode login senyap](/id/open/development/obtain-the-userid-of-a-user-by-using-the-log-free#). |
...
| Nama | Tipe | Deskripsi |
| --- | --- | --- |
| migrationUnionIdList | Map | unionId dari Akun Perusahaan hasil migrasi. |
...
Jika terjadi error saat memanggil API ini, temukan solusinya dengan mencari pesan error di dokumen [Kode error global](/id/open/development/server-api-error-codes-1).

| HttpCode | Kode error | Pesan error | Deskripsi |
| --- | --- | --- | --- |
| 400 | invalid.param | The input parameter cannot be empty. | Parameter masukan tidak boleh kosong. |
| 400 | invalid.unionId | The input parameter unionId cannot be empty. | Parameter masukan unionId tidak boleh kosong. |
