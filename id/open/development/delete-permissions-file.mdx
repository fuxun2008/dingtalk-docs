---
title: "Menghapus izin"
description: "Panggil API ini untuk menghapus izin anggota pada ruang Knowledge Base, node, dan file Drive di DingTalk berdasarkan uuid file, unionId operator, dan id izin."
---

Panggil API ini untuk menghapus izin pengguna dari Knowledge Base, node Knowledge Base, atau file Drive, berdasarkan UUID file, unionId operator, dan ID izin.
...
| Cakupan izin | permission-Storage.Permission.Write-Izin tulis untuk informasi otorisasi penyimpanan organisasi |

### Header permintaan

| Nama | Tipe | Wajib | Deskripsi |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | Ya | Kredensial akses untuk memanggil API ini. Untuk memperoleh nilainya, panggil API [Memperoleh access token aplikasi internal](/id/open/development/obtain-the-access-token-of-an-internal-app). |

### Parameter jalur

| Nama | Tipe | Wajib | Deskripsi |
| --- | --- | --- | --- |
| dentryUuid | String | Ya | UUID file. Untuk memperoleh nilai ini, panggil API [Pencarian File](/id/open/development/search-for-files) atau API [Memperoleh dentryUuid](/id/open/development/api-getuuidbydentryid), lalu baca bidang `dentryUuid` dari respons. |
...
| Nama | Tipe | Wajib | Deskripsi |
| --- | --- | --- | --- |
| roleId | String | Ya | ID peran. Nilai enum:   - **OWNER**: Pemilik. Mencakup cakupan izin berikut:    - PermissionPrivilegeEnum.INFO   - PermissionPrivilegeEnum.LIST   - PermissionPrivilegeEnum.PREVIEW   - PermissionPrivilegeEnum.READ   - PermissionPrivilegeEnum.WRITE   - PermissionPrivilegeEnum.DOWNLOAD   - PermissionPrivilegeEnum.ADD   - PermissionPrivilegeEnum.DELETE   - PermissionPrivilegeEnum.MODIFY   - PermissionPrivilegeEnum.COPY   - PermissionPrivilegeEnum.RENAME   - PermissionPrivilegeEnum.READ_PERMISSION   - PermissionPrivilegeEnum.WRITE_PERMISSION   - PermissionPrivilegeEnum.ASSIGN - **MANAGER**: Pengelola. Mencakup cakupan izin berikut:    - PermissionPrivilegeEnum.INFO   - PermissionPrivilegeEnum.LIST   - PermissionPrivilegeEnum.PREVIEW   - PermissionPrivilegeEnum.READ   - PermissionPrivilegeEnum.WRITE   - PermissionPrivilegeEnum.DOWNLOAD   - PermissionPrivilegeEnum.ADD   - PermissionPrivilegeEnum.DELETE   - PermissionPrivilegeEnum.MODIFY   - PermissionPrivilegeEnum.COPY   - PermissionPrivilegeEnum.RENAME   - PermissionPrivilegeEnum.READ_PERMISSION   - PermissionPrivilegeEnum.WRITE_PERMISSION - **EDITOR**: Editor. Mencakup cakupan izin berikut:    - PermissionPrivilegeEnum.INFO   - PermissionPrivilegeEnum.LIST   - PermissionPrivilegeEnum.PREVIEW   - PermissionPrivilegeEnum.READ   - PermissionPrivilegeEnum.WRITE   - PermissionPrivilegeEnum.DOWNLOAD   - PermissionPrivilegeEnum.ADD - **DOWNLOADER**: Penampil dengan izin unduh. Mencakup cakupan izin berikut:    - PermissionPrivilegeEnum.INFO   - PermissionPrivilegeEnum.LIST   - PermissionPrivilegeEnum.PREVIEW   - PermissionPrivilegeEnum.READ   - PermissionPrivilegeEnum.DOWNLOAD - **READER**: Hanya lihat. Mencakup cakupan izin berikut:    - PermissionPrivilegeEnum.INFO   - PermissionPrivilegeEnum.LIST   - PermissionPrivilegeEnum.PREVIEW |
| members | Array | Ya | Daftar anggota izin. Mendukung hingga 30 anggota. |
...
| id | String | Ya | ID anggota izin:   - Ketika `type=ORG`, nilainya adalah ID organisasi. - Ketika `type=DEPT`, nilainya adalah ID departemen. - Ketika `type=TAG`, nilainya adalah ID tag. - Ketika `type=CONVERSATION`, nilainya adalah ID obrolan. - Ketika `type=USER`, nilainya adalah userId karyawan. |
| corpId | String | Tidak | Organisasi tempat izin tersebut berada.       - Jika ID organisasi disediakan, izin akan otomatis dihapus ketika anggota terkait meninggalkan organisasi. - Jika `memberType` adalah `dept`, ID organisasi wajib diisi. |
    ...
    public static void main(String[] args_) throws Exception {
        ...
        try {
            ...
            if (!com.aliyun.teautil.Common.empty(err.code) && !com.aliyun.teautil.Common.empty(err.message)) {
                // err berisi Atribut code dan message, yang dapat membantu pemecahan masalah
            }
...
Jika sebuah error dikembalikan saat Anda memanggil API ini, lihat dokumentasi [Kode error global](/id/open/development/server-api-error-codes-1) untuk solusinya.
...
| 403 | permissionDenied | %s | Pengguna tidak memiliki izin yang diperlukan. |
| 404 | spaceNotExist | %s | Ruang tersebut tidak ada. |
| 404 | dentryNotExist | %s | File tersebut tidak ada. |
