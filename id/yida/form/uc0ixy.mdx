---
title: "Pemotongan dan unggah gambar (lanjutan)"
description: "Pelajari cara mengimplementasikan pemotongan dan unggah gambar dengan mengemas Komponen kustom di YiDA — mencakup latar belakang kasus, demo, dan langkah-langkah pengembangan low-code secara rinci."
polished: true
---
<Note>
- Komponen ini merupakan enkapsulasi lebih lanjut berdasarkan [Contoh Komponen kustom - Pemotongan dan unggah gambar (menengah)](/id/yida/form/eu0e6k), yang mengurangi biaya penggunaan bagi Pengguna.
...
- Komponen YiDA merilis fitur Komponen kustom yang mendukung pembangunan Komponen secara low-code, tetapi tidak mendukung komponen formulir — hanya Komponen bertipe tampilan yang dapat dibangun.
- Namun, dokumentasi mencatat bahwa formulir yang sudah ada dapat digunakan untuk penyimpanan. Dengan pendekatan ini, Komponen pemotongan dan unggah gambar dibangun dengan membuat formulir baru untuk menyimpan data, sehingga menyimulasikan komponen formulir semu.

## 3. Langkah-langkah

##### 2. Aktifkan fitur Impor schema, dengan cara yang sama seperti desainer formulir YiDA. Untuk detailnya, lihat referensi berikut:
...
##### 3. Tempel schema di bawah ini ke konsol schema desainer Komponen, lalu klik Impor schema.
...
## 4. Prinsip di balik kasus ini

1. Berdasarkan kasus 2, kasus ini terutama memanfaatkan kemampuan menulis kode pada properti konfigurasi "on value change" Komponen. Kode ditulis untuk menghasilkan schema yang secara otomatis membuat formulir untuk menyimpan data.
2. Pengguna hanya perlu mengklik "Aktifkan penyimpanan formulir" untuk menggunakannya.
3. Pemotongan gambar didukung oleh react-cropper open-source (https://www.npmjs.com/package/react-cropper). Karena instalasi dependensi NPM belum didukung, pemuatan dinamis melalui CDN adalah satu-satunya opsi.
