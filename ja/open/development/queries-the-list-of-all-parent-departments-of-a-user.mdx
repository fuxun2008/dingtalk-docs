...
| 名前 | タイプ | 必須 | 例 | 説明 |
| --- | --- | --- | --- | --- |
| access_token | String | はい | bE74xxxx | この API を呼び出すためのアプリ認証情報。   - 社内アプリの場合は、社内アプリの access_token 取得インターフェースで取得します。 - サードパーティ社内アプリの場合は、サードパーティ企業の access_token 取得インターフェースで取得します。 |
...
このインターフェースの呼び出しでエラーが発生した場合は、エラーメッセージをもとに[グローバルエラーコード](/zh/open/development/server-api-error-codes-1)ドキュメントで解決策を検索してください。
