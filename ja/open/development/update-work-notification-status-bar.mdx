...
| 名前 | タイプ | 必須 | 例 | 説明 |
| --- | --- | --- | --- | --- |
| access_token | String | はい | bE74xxxx | この API を呼び出すためのアプリ認証情報。   - 社内アプリの場合は、社内アプリの access_token を取得する API で取得します。 - サードパーティ社内アプリの場合は、サードパーティ企業の access_token を取得する API で取得します。 |
...
| task_id | Number | はい | 11 | 業務通知タスクの ID。[業務通知の送信](/zh/open/development/asynchronous-sending-of-enterprise-session-messages) API を呼び出して task_id パラメータの値を取得します。 |
...
API 呼び出しでエラーが発生した場合は、エラーメッセージをもとに[グローバルエラーコード](/zh/open/development/server-api-error-codes-1)ドキュメントで解決方法を検索してください。
