...
本インターフェースを呼び出すと、指定したカレンダーにアクセス制御を追加できます。
...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | 当該インターフェースを呼び出すためのアクセス認証情報。以下の方法で取得します。   - 社内アプリの場合は、[社内アプリのアクセストークン取得](https://open.dingtalk.io/document/development/obtain-the-access-token-of-an-internal-app#)インターフェースを呼び出して取得します。 - サードパーティ個人アプリの場合は、[ユーザートークン取得](https://open.dingtalk.io/document/development/obtain-user-token#)インターフェースを呼び出して取得します。 |

### パスパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| userId | String | はい | 予定の主催者の unionId。   - 社内アプリの場合は、[ユーザー詳細の照会](https://open.dingtalk.io/document/development/query-user-details#)インターフェースを呼び出して unionid パラメータの値を取得します。 - サードパーティ個人アプリの場合は、[ユーザー連絡先プロフィール取得](https://open.dingtalk.io/document/development/dingtalk-retrieve-user-information#)インターフェースを通じて unionId パラメータの値を取得します。 |
| calendarId | String | はい | 予定が属するカレンダー ID。primary に統一されており、ユーザーのメインカレンダーを表します。 |

### リクエストボディ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| privilege | String | はい | 権限情報。取り得る値：   - **free_busy_reader**：空き状況を確認 - **title_reader**：見出しを確認 - **reader**：詳細を見る - **writer**：作成と編集 |
| sendMsg | Boolean | はい | 権限を付与された人にチャットを送信するかどうか。   - **true**：送信する - **false**：送信しない |
| scope | Object | はい | 権限スコープ。 |
| scopeType | String | はい | 権限タイプ。現在は **user**（ユーザー）のみサポートされています。 |
| userId | String | はい | ユーザー ID。**scopeType** の値が **user** の場合、ユーザーの **unionId** を渡します。  **説明**  1 人につき最大 500 人まで共有相手を追加できます。 |
...
| 名前 | タイプ | 説明 |
| --- | --- | --- |
| privilege | String | 権限情報。取り得る値：   - **free_busy_reader**：空き状況を確認 - **title_reader**：見出しを確認 - **reader**：詳細を見る - **writer**：作成と編集 |
| aclId | String | 権限リソース ID。 |
| scope | Object | 権限スコープ。 |
| scopeType | String | 権限タイプ。現在は **user**（ユーザー）のみサポートされています。 |
| userId | String | ユーザー ID。  **scopeType** の値が **user** の場合、値はユーザーの **unionId** となります。 |
...
当該インターフェースの呼び出しでエラーが発生した場合は、エラーメッセージに基づいて[グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1)ドキュメントで解決策を検索してください。
