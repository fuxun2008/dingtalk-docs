...
| label_ids | Number[] | はい | [1,2,3] | タグリスト。[外部連絡先リストの取得](/zh/open/development/obtain-the-external-contact-list)インターフェースを呼び出してタグ情報を照会できます。  [企業によるカスタムタググループの作成方法](https://tms.dingtalk.io/markets/dingtalk/biaoqianzu)を参照してカスタムタグを追加してください。  1 回の呼び出しで最大 20 個の labelId を渡すことができます。 |
| share_dept_ids | Number[] | いいえ | [420606536] | 共有先の部門 ID。[子部門 ID リストの取得](/zh/open/development/obtain-the-list-of-sub-department-ids)インターフェースを呼び出して取得できます。  1 回の呼び出しで最大 20 個の部門 ID を渡すことができます。 |
| address | String | いいえ | 北京 | 住所。 |
| remark | String | いいえ | 技術責任者 | 表示名。 |
| follower_user_id | String | はい | 023420013645 | 担当者の userId。 |
| name | String | はい | 張xx | 外部連絡先の名前。 |
| user_id | String | はい | 123420013632 | 当該外部連絡先の userId。[外部連絡先リストの取得](/zh/open/development/obtain-the-external-contact-list)インターフェースを呼び出して取得できます。 |
...
本インターフェースの呼び出しでエラーが発生した場合、エラーメッセージに基づいて [グローバルエラーコード](/zh/open/development/server-api-error-codes-1) ドキュメントから解決策を検索してください。
