...
1. サーバー API の [ユーザー作成](/zh/open/development/user-information-creation) を呼び出し、ユーザーを作成します。ユーザーの `userid` を取得します。

   - 社員をルート部門に追加する場合、API パラメータに 1 を渡します。本サンプルではルート部門への追加を採用しています。
   - 社員を特定の部門に追加する場合は、まず [子部門 ID リストの取得](/zh/open/development/obtain-the-list-of-sub-department-ids) API を呼び出して部門 ID を取得し、ユーザー作成 API の部門パラメータとして使用します。
2. ユーザーの `userid` 情報をもとに、サーバー API の [ユーザー詳細の照会](/zh/open/development/query-user-details) を呼び出し、ユーザーの詳細情報を取得します。
3. ユーザーの `userid` 情報をもとに、サーバー API の [ユーザー情報の更新](/zh/open/development/user-information-update) を呼び出し、ユーザー情報を更新します。
4. ユーザーの `userid` 情報をもとに、サーバー API の [ユーザー削除](/zh/open/development/delete-a-user) を呼び出し、ユーザー情報を削除します。
...
### 説明
   ...
   ユーザー詳細取得の戻り値の一部フィールドには一定の要件があります。[ユーザー詳細の照会](/zh/open/development/query-user-details) API ドキュメントの戻りフィールドの説明を参照してください。
