...
ステップ 3：アプリのアクセス認証情報を取得し、[社内アプリの access_token を取得](/ja/open/development/obtain-orgapp-token)します。API 呼び出し時には、アクセストークンで呼び出し元の認証を行います。
...
ステップ 1 で取得したクライアント ID と Client Secret をもとに、アプリのアクセス認証情報を取得し、[社内アプリの access_token を取得](/ja/open/development/obtain-orgapp-token)します。
