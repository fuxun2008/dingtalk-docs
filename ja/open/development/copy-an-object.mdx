...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | このAPIを呼び出すためのアクセス認証情報。以下の方法で取得します：   - 社内アプリの場合は、[社内アプリのaccessTokenを取得](https://open.dingtalk.io/document/development/obtain-the-access-token-of-an-internal-app)APIを呼び出して取得します。 - サードパーティ社内アプリの場合は、[サードパーティアプリが権限付与された企業のaccessTokenを取得](https://open.dingtalk.io/document/development/obtain-the-access-token-of-the-authorized-enterprise-1)APIを呼び出して取得します。 |

### パスパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| spaceId | String | はい | ファイルまたはフォルダーが存在するソーススペースID。[スペースを追加](https://open.dingtalk.io/document/development/add-space)APIを呼び出してidパラメータの値を取得できます。 |
| dentryId | String | はい | コピー対象のファイルまたはフォルダーのID。[ファイルまたはフォルダーのリストを取得](https://open.dingtalk.io/document/development/get-a-list-of-files-or-folders)または[dentryUuidからspaceIdとdentryIdを取得](https://open.dingtalk.io/document/development/api-getdentryidbyuuid)APIを呼び出してidパラメータの値を取得できます。 |

### クエリパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| unionId | String | はい | 操作者のunionId。[ユーザー詳細を取得](https://open.dingtalk.io/document/development/query-user-details)APIを呼び出してunionidパラメータの値を取得できます。 |

### リクエストボディ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| targetSpaceId | String | はい | 保存先のターゲットスペースID。[スペースを追加](https://open.dingtalk.io/document/development/add-space)APIを呼び出してidパラメータの値を取得できます。 |
| targetFolderId | String | はい | 保存先の親ディレクトリID。[ファイルまたはフォルダーのリストを取得](https://open.dingtalk.io/document/development/get-a-list-of-files-or-folders)APIを呼び出してidパラメータの値を取得できます。 |
...
このAPI呼び出しでエラーが発生した場合は、エラーメッセージに基づいて[グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1)ドキュメントから解決策をご確認ください。
