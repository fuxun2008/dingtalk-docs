...
前提条件：[アプリの作成と設定](/zh/open/dingstart/create-and-configure-an-application)のフローを完了していること。

ステップ 1：アプリ認証情報を取得し、クライアント ID と Client Secret を取得します。

ステップ 2：API 権限をリクエストし、スマート会議室関連の API 権限を申請します。

ステップ 3：アプリのアクセス認証情報を取得します。[社内アプリのアクセストークン取得](/zh/open/development/obtain-the-access-token-of-an-internal-app)を参照してください。API を呼び出す際は、アクセストークンによって呼び出し元の身元を認証します。

ステップ 4：会議室関連の API を呼び出します。

1. サーバー API の [会議室の作成](/zh/open/development/create-a-meeting-room) インターフェースを呼び出し、会議室の戻り値 `result` フィールド、すなわち会議室 ID を取得します。
2. 会議室 ID に基づき、サーバー API の [会議室情報の更新](/zh/open/development/update-meeting-room-information) インターフェースを呼び出し、会議室情報の更新操作を実行します。
3. サーバー API の [会議室リストの照会](/zh/open/development/check-the-meeting-room-list) インターフェースを呼び出し、会議室リストのコンテンツを取得します。
4. 会議室 ID に基づき、サーバー API の [会議室詳細の照会](/zh/open/development/check-meeting-room-details) インターフェースを呼び出し、特定の会議室の詳細情報を取得します。
5. 会議室 ID に基づき、サーバー API の [会議室の削除](/zh/open/development/delete-a-meeting-room) インターフェースを呼び出し、会議室の削除操作を実行します。
...
ステップ 1 で取得したクライアント ID と Client Secret を使用し、アプリのアクセス認証情報を取得します。[社内アプリのアクセストークン取得](/zh/open/development/obtain-the-access-token-of-an-internal-app)を参照してください。
