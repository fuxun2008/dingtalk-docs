---
title: "フィルタービュー条件の削除"
description: "フィルタービューで特定列のみ絞り込み条件を削除して他の絞り込みを保持します"
---
...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| workbookId | String | 必須 | スプレッドシートファイルの ID です。[ナレッジベース API](/ja/open/development/knowledge-base-overview) から返される `nodeId(dentryUuid)` がスプレッドシートの `workbookId` に該当します。[ノードの取得](https://open.dingtalk.com/document/orgapp/get-node) および [ナレッジベースドキュメントの作成](/ja/open/development/create-team-space-document) API を呼び出して取得できます。 |
| sheetId | String | 必須 | ワークシートの ID または名称です。[全ワークシートの取得](/ja/open/development/obtain-all-worksheets) API を呼び出して、id または name パラメータの値を取得できます。 |
...
API 呼び出し時にエラーが発生した場合は、エラーメッセージをもとに [グローバルエラーコード](/ja/open/development/server-api-error-codes-1) ドキュメントから解決策を確認してください。
