...
| task_id | Number | はい | 256271667526 | チャットを送信したときに DingTalk から返されたタスクIDです。[業務通知を送信する](/zh/open/development/asynchronous-sending-of-enterprise-session-messages)APIを呼び出して task_id パラメータの値を取得します。  **説明**  24時間以内のタスクのみ照会できます。 |
...
このAPIの呼び出し時にエラーが発生した場合、エラーメッセージをもとに[グローバルエラーコード](/zh/open/development/server-api-error-codes-1)ドキュメントから解決策を検索してください。
