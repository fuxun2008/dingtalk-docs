...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| spaceId | String | はい | スペース ID。[スペースを追加](https://open.dingtalk.io/document/development/add-space#) API を呼び出して id パラメータ値を取得します。 |
| dentryId | String | はい | ファイル ID。[ファイルまたはフォルダーリストを取得](https://open.dingtalk.io/document/development/get-a-list-of-files-or-folders#) または [dentryUuid に基づいて spaceId を取得](https://open.dingtalk.io/document/development/api-getdentryidbyuuid#) API を呼び出して id パラメータ値を取得します。 |
...
| version | Long | いいえ | ファイルのバージョン番号。[ファイルバージョンリストを取得](https://open.dingtalk.io/document/development/obtains-a-list-of-file-versions#) API を呼び出して version を取得します。 |
...
本 API の呼び出しでエラーが発生した場合、エラーメッセージに基づいて [グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1) ドキュメントから解決策を確認できます。
