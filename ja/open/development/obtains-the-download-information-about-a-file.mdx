...
<Note>
ファイルサイズが大きい場合、リソースを分割して取得する必要があります。上記サンプルに headers フィールドを追加できます：`headers.put("Range","bytes=0-499");`。詳細は [Range を指定して部分コンテンツをダウンロード](/zh/open/development/use-range-download-a-part-of-content) を参照してください。
</Note>
