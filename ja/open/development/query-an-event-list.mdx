...
- デフォルトでは削除済みの予定は返されません。返却が必要な場合は `showDeleted=true` を設定してください。返された予定が削除済みの場合、[単一予定の詳細を照会](https://open.dingtalk.io/document/development/query-details-about-an-event#) API を呼び出すと **instanceNotExist** エラーが返されます。
...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | 必須 | 本 API を呼び出すためのアクセス認証情報。以下の方法で取得します：   - 社内アプリの場合、[社内アプリのアクセストークンを取得](https://open.dingtalk.io/document/development/obtain-the-access-token-of-an-internal-app#) API を呼び出して取得します。 - サードパーティ社内アプリの場合、[サードパーティアプリが認可された企業のアクセストークンを取得](https://open.dingtalk.io/document/development/obtain-the-access-token-of-the-authorized-enterprise-1#) API を呼び出して取得します。 - サードパーティ個人アプリの場合、[ユーザートークンを取得](https://open.dingtalk.io/document/development/obtain-user-token#) API を呼び出して取得します。 |

### パスパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| userId | String | 必須 | 照会対象ユーザーの unionId。   - 社内アプリおよびサードパーティ社内アプリの場合、[ユーザー詳細の照会](https://open.dingtalk.io/document/development/query-user-details#) API を呼び出して unionid パラメータ値を取得します。 - サードパーティ個人アプリの場合、[ユーザー連絡先のプロフィールを取得](https://open.dingtalk.io/document/development/dingtalk-retrieve-user-information#) API を呼び出して unionId パラメータ値を取得します。 |
...
| maxAttendees | Integer | 任意 | 各予定の参加者照会件数。デフォルトは 100、最大は 100。 参加者が 100 人を超え、全参加者を取得する必要がある場合は、[予定参加者を取得](https://open.dingtalk.io/document/development/get-the-participants-of-a-schedule#) API を使用して取得してください。 |
...
本 API の呼び出し時にエラーが報告された場合、エラーメッセージをもとに[グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1)ドキュメントから解決方法を検索できます。
