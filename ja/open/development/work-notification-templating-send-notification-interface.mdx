...
| data | String | いいえ | `{"name":"淘宝 6","name2":"http://www.taobao.com"}` | チャットテンプレートの動的パラメータ代入データ。**説明**  key と value はいずれも文字列形式です。 |
...
本 API の呼び出し時にエラーが発生した場合、エラーメッセージをもとに[グローバルエラーコード](/zh/open/development/server-api-error-codes-1)ドキュメントから解決方法を検索できます。
