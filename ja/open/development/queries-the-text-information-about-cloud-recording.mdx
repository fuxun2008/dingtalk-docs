...
description: "ビデオ会議のクラウド録画から自動変換されたテキスト情報を API で取得します"
---
...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| conferenceId | String | いいえ | 会議 ID。[ビデオ会議を作成](/ja/open/development/create-a-video-conference#) API を呼び出して conferenceId パラメータの値を取得できます。 |

### クエリパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| unionId | String | いいえ | ユーザーの unionId。[ユーザー詳細を取得](/ja/open/development/query-user-details#) API を呼び出して unionid パラメータの値を取得できます。 |
...
本 API の呼び出しでエラーが発生した場合、エラーメッセージをもとに [グローバルエラーコード](/ja/open/development/server-api-error-codes-1) ドキュメントから解決策を検索できます。
