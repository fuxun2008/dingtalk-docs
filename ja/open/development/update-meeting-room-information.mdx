---
title: "会議室情報の更新"
description: "スマート会議室アプリの管理者が会議室名、定員、位置情報、設備などを更新し利用状態を制御できます"
---
...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | この API を呼び出す際のアクセス認証情報。以下の方法で取得します。   - 社内アプリの場合は、[社内アプリの accessToken を取得](/ja/open/development/obtain-the-access-token-of-an-internal-app) を呼び出して取得します。 - サードパーティ社内アプリの場合は、[サードパーティアプリの認可企業の accessToken を取得](https://open.dingtalk.com/document/development/obtain-the-access-token-of-the-authorized-enterprise-1) を呼び出して取得します。 |

### リクエストボディ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| unionId | String | はい | 操作者の unionId。[ユーザー詳細の照会](/ja/open/development/query-user-details) を呼び出して取得します。 |
| roomId | String | はい | 会議室 ID。[会議室リストの照会](/ja/open/development/check-the-meeting-room-list) を呼び出して取得します。 |
...
| groupId | Long | いいえ | 会議室が属するグループ ID。[会議室グループリストの照会](/ja/open/development/query-meeting-rooms-groups) を呼び出して取得します。 |
| reservationAuthority | Object | いいえ | roomStatus が「一部権限」の場合に設定する、会議室の予約権限。 |
| authorizedMembers | Array | いいえ | 権限を持つユーザーリスト。 |
| memberId | String | いいえ | ユーザーの ID。memberType の設定により異なります。   - **メンバー**：ユーザーの unionId。[ユーザー詳細の照会](/ja/open/development/query-user-details) を呼び出して取得します。 - **部門**：部門 ID、dept_id。[部門リストの取得](https://open.dingtalk.com/document/development/obtain-the-department-list-v2) を呼び出して dept_id のパラメータ値を取得します。 |
