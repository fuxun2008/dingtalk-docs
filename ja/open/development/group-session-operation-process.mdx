...
ステップ 3：アプリのアクセス認証情報を取得します。[社内アプリの access_token を取得](/zh/open/development/obtain-the-access-token-of-an-internal-app)。

ステップ 4：サーバーサイドのグループチャット関連 API を呼び出します。

1. サーバー API のグループ作成 API を呼び出してグループチャットを作成し、グループチャットの `chatId` を取得します。
2. グループチャットの `chatId` に基づき、サーバー API の [グループ情報の取得](/zh/open/development/obtain-a-group-session) API を呼び出して、グループチャット情報を取得します。
3. グループチャットの chatId に基づき、グループチャット管理操作を行います。

   - グループチャットの `chatId` に基づき、新版サーバー API の [グループチャットの OpenConversationId を取得](/zh/open/development/obtain-group-openconversationid) API を呼び出して、グループチャットの `openConversationId` を取得します。
   ...
   - グループチャットの `chatId` に基づき、サーバー API の [メンバー間の個別チャットを禁止](/zh/open/development/set-private-chat) API を呼び出して、グループチャットメンバー間の個別チャットの可否を設定します。
   - グループチャットの `chatId` に基づき、サーバー API の [グループメンバーのグループ名を更新](/zh/open/development/set-a-group-nickname) API を呼び出して、グループチャットメンバーの名前を設定します。
   - グループチャットの `chatId` に基づき、サーバー API の [グループ参加 QR コードリンクの取得](/zh/open/development/obtain-a-qr-code-link) API を呼び出します。他の企業メンバーはそのリンクをクリックすることで、グループチャットへの参加をリクエストできます。
4. グループチャットの `chatId` に基づき、サーバー API のグループ更新 API を呼び出して、グループチャット情報を更新します。

## 前提条件

[アプリの作成と設定](/zh/open/dingstart/create-and-configure-an-application) のフローを完了していること。
...
ステップ 1 で取得したクライアント ID と Client Secret に基づき、アプリのアクセス認証情報を取得します。[社内アプリの access_token を取得](/zh/open/development/obtain-the-access-token-of-an-internal-app)。
