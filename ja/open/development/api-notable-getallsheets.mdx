---
title: "全データテーブルの取得"
description: "AIテーブルベースのすべてのデータテーブルを取得し、テーブル一覧と構成を確認できます"
---
...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | 本インターフェイスを呼び出すためのアクセス認証情報です。[社内アプリの accessToken を取得](/ja/open/development/obtain-the-access-token-of-an-internal-app#)インターフェイスを呼び出して取得します。 |

### パスパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| baseId | String | はい | AIテーブル ID です。取得方法は[データ構造](/ja/open/development/data-structure#)をご参照ください。 |

### クエリパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| operatorId | String | はい | 操作者の unionId です。以下の 2 つの方法で取得できます。   - [ユーザー詳細の照会](/ja/open/development/query-user-details#)インターフェイスを呼び出して取得します。 - [サイレントログインコードによるユーザー情報の取得](/ja/open/development/obtain-the-userid-of-a-user-by-using-the-log-free#)インターフェイスを呼び出して取得します。 |
...
本インターフェイスの呼び出しでエラーが発生した場合は、エラーメッセージをもとに[グローバルエラーコード](/ja/open/development/server-api-error-codes-1)ドキュメントで解決策をご確認ください。
