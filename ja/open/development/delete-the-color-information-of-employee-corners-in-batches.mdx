...
| 名前 | タイプ | 必須 | 例 | 説明 |
| --- | --- | --- | --- | --- |
| roleIds | String | 必須 | 1507113578 | ロール roleId のリスト。[ロールリスト取得](/zh/open/development/obtains-a-list-of-enterprise-roles)インターフェースを呼び出して取得できます。リストの最大長は 20 で、複数の roleId は半角カンマ（,）で区切ります。 |
| userIds | String | 必須 | user100 | 社員の userid。[電話番号でユーザーを検索](/zh/open/development/query-users-by-phone-number)を呼び出して userId を取得できます。リストの最大長は 100 で、複数の userId は半角カンマ（,）で区切ります。 |
...
本インターフェースの呼び出しでエラーが発生した場合は、エラーメッセージをもとに[グローバルエラーコード](/zh/open/development/server-api-error-codes-1)ドキュメントで解決策をご確認ください。
