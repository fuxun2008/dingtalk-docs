...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | 本 API を呼び出す際のアクセス認証情報です。[社内アプリのアクセストークンの取得](https://open.dingtalk.io/document/development/obtain-the-access-token-of-an-internal-app) API から取得できます。 |
...
| icon | String | いいえ | グループのプロフィール写真。mediaId を渡す必要があり、[メディアファイルのアップロード](https://open.dingtalk.io/document/development/upload-media-files) API から取得できます。 |
...
本 API の呼び出しでエラーが発生した場合は、エラーメッセージに基づいて[グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1)ドキュメントで解決策をご確認ください。
