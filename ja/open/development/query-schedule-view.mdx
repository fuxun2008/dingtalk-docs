...
- 本 API と**予定リストの照会** API との違いは、繰り返し予定の取り扱いにあります。本 API は繰り返しルールに基づき、照会期間内のすべての繰り返し予定を展開しますが、データベース内に実際のインスタンスを生成することはありません。例えば、1月1日から開始される毎日繰り返しの予定がある場合、照会時点が1月4日で、照会期間が1月1日から1月10日であるとき、本 API は1月1日から1月10日までのすべての繰り返し予定を返しますが、[予定リストの照会](https://open.dingtalk.io/document/development/query-an-event-list#) API では1月4日以降の予定は返されません。
...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | 本 API を呼び出すためのアクセス認証情報。以下の方法で取得します。   - 社内アプリの場合、[社内アプリの accessToken を取得](https://open.dingtalk.io/document/development/obtain-the-access-token-of-an-internal-app#) API を呼び出して取得します。 - サードパーティ社内アプリの場合、[サードパーティアプリ認可企業の accessToken を取得](https://open.dingtalk.io/document/development/obtain-the-access-token-of-the-authorized-enterprise-1#) API を呼び出して取得します。 - サードパーティ個人アプリの場合、[ユーザートークンを取得](https://open.dingtalk.io/document/development/obtain-user-token#) API を呼び出して取得します。 |

### パスパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| userId | String | はい | 照会対象ユーザーの unionId。   - 社内アプリおよびサードパーティ社内アプリの場合、[ユーザー詳細の照会](https://open.dingtalk.io/document/development/query-user-details#) API を呼び出して unionid パラメータ値を取得します。 - サードパーティ個人アプリの場合、[ユーザーの連絡先プロフィールを取得](https://open.dingtalk.io/document/development/dingtalk-retrieve-user-information#) API を呼び出して unionId パラメータ値を取得します。 |
| calendarId | String | はい | 予定が属するカレンダー ID。[カレンダーの照会](https://open.dingtalk.io/document/development/query-a-calendar#) API を呼び出して現在のユーザーの予定を照会します。**primary** キーワードを使用すると、現在のユーザーのプライマリカレンダーを表します。 |
...
| maxAttendees | Integer | いいえ | 各予定の参加者照会件数。デフォルト 100、最大 100。参加人数が 100 人を超え、すべての参加者を取得する必要がある場合は、[予定参加者の取得](https://open.dingtalk.io/document/development/get-the-participants-of-a-schedule#) API を使用して取得してください。 |
...
本 API の呼び出し時にエラーが発生した場合は、エラーメッセージをもとに [グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1) ドキュメントから解決策を検索してください。
