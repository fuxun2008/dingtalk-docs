...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| creatorUnionId | String | はい | 会議予約の作成者のunionId。 |
| scheduleConferenceId | String | はい | 会議予約ID。[会議予約の作成](https://open.dingtalk.io/document/development/create-appointment-meeting#) APIを呼び出して、戻り値の `scheduleConferenceId` フィールドから取得できます。 |
| title | String | はい | 会議予約のタイトル。最大長は50文字までです。 |
| startTime | Long | はい | 会議予約の開始時刻。ミリ秒単位のUTCタイムスタンプ。 |
| endTime | Long | はい | 会議予約の終了時刻。ミリ秒単位のUTCタイムスタンプ。 |
...
このAPIの呼び出しでエラーが発生した場合、エラーメッセージに基づき [グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1) ドキュメントで解決方法を検索できます。

| HttpCode | エラーコード | エラーメッセージ | 説明 |
| --- | --- | --- | --- |
| 400 | paramsError | error:%s | パラメータエラー |
| 400 | permissionError | permissionError:%s | この会議を操作する権限がありません |
| 400 | resourceCancel | Schedule conference has canceled | 会議予約が既にキャンセルされています |
