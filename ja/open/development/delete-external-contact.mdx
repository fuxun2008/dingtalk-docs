...
| 名前 | タイプ | 必須 | 例 | 説明 |
| --- | --- | --- | --- | --- |
| user_id | String | 必須 | 0125xxxx0251487 | 削除対象の外部連絡先の userId。[外部連絡先リストの取得](/zh/open/development/obtain-the-external-contact-list)インターフェースを呼び出して userid パラメータの値を取得できます。 |
...
本インターフェースの呼び出し時にエラーが発生した場合は、エラーメッセージをもとに[グローバルエラーコード](/zh/open/development/server-api-error-codes-1)ドキュメントで解決方法を検索してください。
