...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | 必須 | 本インターフェースを呼び出すためのアクセス認証情報。以下の方法で取得します。   - 社内アプリの場合は、[社内アプリの access_token を取得する](https://open.dingtalk.io/document/development/obtain-the-access-token-of-an-internal-app#)インターフェースを呼び出して取得します。 - サードパーティ社内アプリの場合は、[サードパーティアプリの認可企業の access_token を取得する](https://open.dingtalk.io/document/development/obtain-the-access-token-of-the-authorized-enterprise-1#)インターフェースを呼び出して取得します。 - サードパーティ個人アプリの場合は、[ユーザートークンを取得する](https://open.dingtalk.io/document/development/obtain-user-token#)インターフェースを呼び出して取得します。 |

### パスパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| userId | String | 必須 | 本リクエストのリソースが帰属するユーザーの unionId。   - 社内アプリおよびサードパーティ社内アプリの場合は、[ユーザー詳細を照会する](https://open.dingtalk.io/document/development/query-user-details#)インターフェースを呼び出して unionid パラメータの値を取得します。 - サードパーティ個人アプリの場合は、[ユーザーの連絡先プロフィールを取得する](https://open.dingtalk.io/document/development/dingtalk-retrieve-user-information#)インターフェースを呼び出して unionId パラメータの値を取得します。 |

### リクエストボディ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| userIds | Array of String | 必須 | 照会対象ユーザーの unionId。   - 社内アプリおよびサードパーティ社内アプリの場合は、[ユーザー詳細を照会する](https://open.dingtalk.io/document/development/query-user-details#)インターフェースを呼び出して unionid パラメータの値を取得します。 - サードパーティ個人アプリの場合は、[ユーザーの連絡先プロフィールを取得する](https://open.dingtalk.io/document/development/dingtalk-retrieve-user-information#)インターフェースを呼び出して unionId パラメータの値を取得します。       ユーザーリストの最大長は 20 です。 |
