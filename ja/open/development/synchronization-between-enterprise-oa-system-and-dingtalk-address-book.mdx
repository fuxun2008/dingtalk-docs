...
[アプリの作成](/ja/open/dingstart/create-and-configure-an-application)フローを完了します。
...
### 説明
   ...
   2. 企業 OA システムの連絡先にロールを追加する場合、企業 OA システム内のロール情報に基づいて行います。
      ...
      1. サーバー API の [ロールグループの作成](/ja/open/development/add-a-role-group) インターフェースを呼び出し、ロールグループ情報を作成します。
      ...
      2. サーバー API のロール作成インターフェースを呼び出し、企業のロール情報を作成します。
         ...
         ```java lines
           public void addRole() throws ApiException {
                 ...
                 req.setRoleName("1019テストロール");
                 req.setGroupId(3168010875L);
                 OapiRoleAddRoleResponse rsp = client.execute(req, getAccessToken());
                 System.out.println(rsp.getBody());
             }
         ```
   3. [ユーザーの作成](/ja/open/development/user-information-creation) インターフェースを呼び出し、企業 OA システム内の各部門の社員を DingTalk の対応する部門に作成・追加します。本ステップでユーザーを追加する際、同時に当該社員のロール情報を追加することができます。
