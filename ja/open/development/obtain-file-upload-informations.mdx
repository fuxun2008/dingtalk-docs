...
- ファイルアップロード時に入力パラメータとして spaceId フィールドを指定する場合は、[ファイルアップロード情報を取得](https://open.dingtalk.io/document/development/obtain-storage-upload-information#) API を使用してください。
...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| parentDentryUuid | String | はい | 親ノードの dentryUuid。[ファイル検索](https://open.dingtalk.io/document/development/search-for-files#) または [dentryUuid 情報を取得](https://open.dingtalk.io/document/development/api-getuuidbydentryid#) API を呼び出し、レスポンスパラメータの `dentryUuid` フィールドを取得できます。    スペースのルートディレクトリの場合は、スペースルートディレクトリの dentryUuid を指定します。 |

### クエリパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| unionId | String | はい | 操作者の unionId。[ユーザー詳細を照会](https://open.dingtalk.io/document/development/query-user-details#) API を呼び出して取得します。 |
