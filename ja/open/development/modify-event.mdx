...
- 予定参加者の追加と削除：[予定参加者を追加](https://open.dingtalk.io/document/development/add-schedule-participant#)および[予定参加者を削除](https://open.dingtalk.io/document/development/delete-schedule-participant#)インターフェースの使用を推奨します。
...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| userId | String | 必須 | 予定主催者の unionId。   - 社内アプリおよびサードパーティ社内アプリの場合、[ユーザー詳細を照会](https://open.dingtalk.io/document/development/query-user-details#)インターフェースを呼び出して unionid パラメータ値を取得します。 - サードパーティ個人アプリの場合、[ユーザー連絡先プロフィールを取得](https://open.dingtalk.io/document/development/dingtalk-retrieve-user-information#)インターフェースを呼び出して unionId パラメータ値を取得します。 |
| calendarId | String | 必須 | 予定が属するカレンダー ID。primary に統一されており、ユーザーのメインカレンダーを示します。 |
| eventId | String | 必須 | 予定 ID。[予定リストを照会](https://open.dingtalk.io/document/development/query-an-event-list#)インターフェースを呼び出して id パラメータ値を取得できます。 |
...
このインターフェースの呼び出しでエラーが発生した場合、エラーメッセージに基づき[グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1)ドキュメントから解決策を検索できます。
