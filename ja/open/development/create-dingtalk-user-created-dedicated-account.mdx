...
| name | String | はい | 張三 | 社員名。最大 80 文字。 |
...
| extension | String | いいえ | `{"爱好":"旅游","年龄":"24"}` | 拡張属性。複数の属性を設定可能。最大長は 2000 文字。**説明**   - 携帯電話には最大 10 個の拡張属性しか表示できません。 - このパラメータを使用する前に、[DingTalk 管理コンソール](https://oa.dingtalk.io/index_new.htm#/setting/contactInfo) で該当属性を追加してから、API を呼び出して値を設定してください。 - このフィールドの値はリンクタイプの入力に対応し、リンクは変数ワイルドカードによる自動置換にも対応しています。現在対応しているワイルドカードは userid、corpid です。例：`{"爱好":"[爱好](http://www.dingtalk.io?userid=#userid#&corpid=#corpid#)"}`。 |
...
| avatarMediaId | String | いいえ | @lALPDfmVUw19YdrNA-jNA-g | 本組織の企業アカウントを作成する際に指定可能なプロフィール写真の MediaId。jpg/png のみ対応。  [メディアファイルをアップロードする](/zh/open/development/upload-media-files) API を呼び出して取得できます。  **説明**  企業アカウントのみに適用されます。 |
...
この API の呼び出しでエラーが発生した場合は、エラーメッセージを基に [グローバルエラーコード](/zh/open/development/server-api-error-codes-1) ドキュメントから解決策を確認してください。
