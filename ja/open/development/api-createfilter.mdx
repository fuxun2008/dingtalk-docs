---
title: "フィルターを作成する"
description: "スプレッドシート上にグローバルフィルターを作成し、テーブルデータを特定条件で行レベルで絞り込み表示します"
---
...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| workbookId | String | はい | スプレッドシートファイルの ID。[ナレッジベース API](/ja/open/development/knowledge-base-overview) が返す `nodeId(dentryUuid)` がスプレッドシートの `workbookId` に相当し、[ノードを取得](https://open.dingtalk.com/document/orgapp/get-node) および [ナレッジベースドキュメントを作成](/ja/open/development/create-team-space-document) API を呼び出して取得できます。 |
| sheetId | String | はい | ワークシートの ID または名前。[すべてのワークシートを取得](/ja/open/development/obtain-all-worksheets) API を呼び出して id または name パラメータの値を取得します。 |

### クエリパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| operatorId | String | はい | 操作者の unionId。   - [サイレントログインコードでユーザー情報を取得](/ja/open/development/obtain-the-userid-of-a-user-by-using-the-log-free) API を呼び出して unionid パラメータの値を取得できます。 - [ユーザー詳細を取得](/ja/open/development/query-user-details) API を呼び出して unionid パラメータの値を取得できます。       操作者に権限がない場合、API は `The operator has no permission` というエラーを返します。 |
...
API の呼び出しでエラーが発生した場合は、エラーメッセージをもとに [グローバルエラーコード](/ja/open/development/server-api-error-codes-1) ドキュメントで解決方法を検索してください。
