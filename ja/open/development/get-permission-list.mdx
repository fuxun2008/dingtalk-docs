...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | 本インターフェースを呼び出す際のアクセス認証情報。[社内アプリの access_token を取得する](https://open.dingtalk.io/document/development/obtain-the-access-token-of-an-internal-app)インターフェースを呼び出して取得できます。 |

### パスパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| dentryUuid | String | はい | ファイル uuid。[ファイル検索](https://open.dingtalk.io/document/development/search-for-files)インターフェース、または [dentryUuid 情報の取得](https://open.dingtalk.io/document/development/api-getuuidbydentryid)インターフェースを呼び出し、戻りパラメータの dentryUuid フィールドを取得します。 |

### クエリパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| unionId | String | はい | ユーザーの unionId。[ユーザー詳細の取得](https://open.dingtalk.io/document/development/query-user-details)インターフェースを呼び出して取得します。 |
...
本インターフェースの呼び出しでエラーが発生した場合は、エラーメッセージを基に[グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1)ドキュメントで解決策を検索してください。
