...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | API 認証情報。[ユーザートークン取得](https://open.dingtalk.io/document/development/obtain-user-token#)インターフェースを呼び出して取得します。  ユーザートークン取得のリクエストで使用する入力パラメータ `code` は、リンク構築の過程で scope の範囲を必ずユーザー ID と組織 ID とする必要があります。すなわち `scope=openid corpid` です。 |
...
| logo | String | いいえ | テンプレートロゴ。[メディアファイルアップロード](https://open.dingtalk.io/document/development/upload-media-files#)インターフェースを呼び出して取得できます。画像アップロード後の mediaId。画像サイズ：240\*240、1 枚あたり 1 MB 以内。 |
| orgName | String | いいえ | テンプレートの企業名。 |
| customDoc | String | いいえ | テンプレートのカスタムテキストコンテンツ。 |
| bgUrl | String | いいえ | テンプレートのカスタム背景 URL。 |
| bgImgList | Array of String | いいえ | 画像 mediaId。[メディアファイルアップロード](https://open.dingtalk.io/document/development/upload-media-files#)インターフェースを呼び出して取得できます。最大 9 枚までアップロード可能で、1 枚あたり 5 MB 以内。 |
...
| deviceUnionIds | Array of String | いいえ | デバイスの unionId。本パラメータは、[デバイスと会議室の紐付け変更](https://open.dingtalk.io/document/development/org-event-overview#)イベントの購読から取得します。 |
| groupIds | Array of Long | いいえ | グループ分け ID。[会議室グループ分けリスト取得](https://open.dingtalk.io/document/development/query-meeting-rooms-groups#)インターフェースを呼び出して取得できます。 |
| roomIds | Array of String | いいえ | 会議室 ID。[会議室リスト取得](https://open.dingtalk.io/document/development/check-the-meeting-room-list#)インターフェースを呼び出して取得できます。 |
...
本インターフェースの呼び出しでエラーが発生した場合は、エラーメッセージをもとに[グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1)ドキュメントから解決方法を確認してください。
