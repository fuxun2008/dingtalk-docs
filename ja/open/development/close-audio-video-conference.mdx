---
title: "ビデオ会議を閉じる"
description: "ビデオ会議の主催者のみが会議を終了させることができるようにするインターフェースです"
---
...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | 本インターフェースを呼び出す際のアクセス認証情報。以下の方法で取得します：   - 社内アプリの場合は、[社内アプリの access_token を取得](/ja/open/development/obtain-the-access-token-of-an-internal-app#) インターフェースを呼び出して取得します。 - サードパーティ社内アプリの場合は、[サードパーティアプリに権限付与された企業の access_token を取得](https://open.dingtalk.com/document/development/obtain-the-access-token-of-the-authorized-enterprise-1#) インターフェースを呼び出して取得します。 |

### パスパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| conferenceId | String | はい | 会議 ID。[ビデオ会議を作成](/ja/open/development/create-a-video-conference#) インターフェースを呼び出して conferenceId パラメータ値を取得できます。 |

### クエリパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| unionId | String | はい | 社員が現在の開発者の企業アカウント範囲内における一意の識別子。[ユーザー詳細を照会](/ja/open/development/query-user-details#) インターフェースを呼び出して unionid パラメータ値を取得できます。 |
