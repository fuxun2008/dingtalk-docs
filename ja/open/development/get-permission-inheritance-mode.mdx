...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | 本インターフェースを呼び出すためのアクセス認証情報です。[社内アプリのアクセストークン取得](https://open.dingtalk.io/document/development/obtain-the-access-token-of-an-internal-app)インターフェースを呼び出して取得できます。 |

### パスパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| dentryUuid | String | はい | ファイルの uuid です。[ファイル検索](https://open.dingtalk.io/document/development/search-for-files)または [dentryUuid 情報の取得](https://open.dingtalk.io/document/development/api-getuuidbydentryid)インターフェースを呼び出し、レスポンスパラメータの `dentryUuid` フィールドを取得できます。 |

### クエリパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| unionId | String | はい | ユーザーの unionId です。[ユーザー詳細の照会](https://open.dingtalk.io/document/development/query-user-details)インターフェースを呼び出し、unionid パラメータの値を取得できます。 |
...
本インターフェースの呼び出しでエラーが発生した場合、エラーメッセージに基づいて[グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1)ドキュメントから解決策をご確認ください。
