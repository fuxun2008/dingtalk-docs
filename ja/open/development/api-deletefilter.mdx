...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| workbookId | String | はい | スプレッドシートファイル ID です。[ナレッジベース API](https://open.dingtalk.io/document/orgapp/knowledge-base-overview) が返す `nodeId(dentryUuid)` がスプレッドシートの `workbookId` です。[ノードの取得](https://open.dingtalk.io/document/orgapp/get-node) および [ナレッジベースドキュメントの作成](https://open.dingtalk.io/document/orgapp/create-team-space-document) API を呼び出して取得できます。 |
| sheetId | String | はい | ワークシートの ID または名前です。[すべてのワークシートを取得](https://open.dingtalk.io/document/orgapp/obtain-all-worksheets) API を呼び出して id または name パラメータ値を取得できます。 |

### クエリパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| operatorId | String | はい | 操作者の unionId。   - [サイレントログインコードによるユーザー情報の取得](https://open.dingtalk.io/document/orgapp/obtain-the-userid-of-a-user-by-using-the-log-free) API で unionid パラメータ値を取得できます。 - [ユーザー詳細のクエリ](https://open.dingtalk.io/document/orgapp/query-user-details) API を呼び出して unionid パラメータ値を取得できます。       操作者に権限がない場合、API はエラー `The operator has no permission` を返します。 |
...
この API の呼び出しでエラーが発生した場合は、エラーメッセージに基づき [グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1) ドキュメントで解決方法を確認してください。
