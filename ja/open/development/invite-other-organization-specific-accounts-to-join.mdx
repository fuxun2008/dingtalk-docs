...
2. 他企業側で API【[企業アカウントの複数組織参加を許可する](/zh/open/development/authorize-a-dedicated-account-to-join-multiple-organizations)】を呼び出し、他企業の企業アカウントが自組織に参加することを許可する必要があります。権限付与が完了すると、他企業が自企業に対し、他企業の企業アカウントを自企業へ参加させることを許可したことを意味します。
...
| name | String | 必須 | 张三 | 社員名。最大 80 文字。 |
...
| extension | String | 任意 | `{"爱好":"旅游","年龄":"24"}` | 拡張属性。複数の属性を設定可能で、最大 2000 文字。**説明**   - モバイル上では最大 10 個の拡張属性まで表示できます。 - 本パラメータを使用する前に、[DingTalk 管理者画面](https://oa.dingtalk.io/index_new.htm#/setting/contactInfo) で当該属性を追加してから API を呼び出して値を設定してください。 - 本フィールドの値にはリンクタイプの指定が可能で、リンク内では変数ワイルドカードによる自動置換に対応しています。現在対応しているワイルドカードは userid、corpid です。例：`{"爱好":"[爱好](http://www.dingtalk.io?userid=#userid#&corpid=#corpid#)"}`。 |
...
本 API の呼び出し時にエラーが発生した場合は、エラーメッセージをもとに [グローバルエラーコード](/zh/open/development/server-api-error-codes-1) ドキュメントから解決策をご確認ください。
