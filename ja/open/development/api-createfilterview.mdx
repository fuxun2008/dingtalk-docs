...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| workbookId | String | はい | スプレッドシートファイル ID。[ナレッジベース API](https://open.dingtalk.io/document/orgapp/knowledge-base-overview) が返す `nodeId(dentryUuid)` がスプレッドシートの `workbookId` に該当します。[ノードを取得する](https://open.dingtalk.io/document/orgapp/get-node) および [ナレッジベースドキュメントを作成する](https://open.dingtalk.io/document/orgapp/create-team-space-document) API を呼び出して取得できます。 |
| sheetId | String | はい | ワークシートの ID または名前。[すべてのワークシートを取得する](https://open.dingtalk.io/document/orgapp/obtain-all-worksheets) API を呼び出して id または name の値を取得できます。 |

### クエリパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| operatorId | String | はい | 操作者の unionId。   - [サイレントログインコードによりユーザー情報を取得する](https://open.dingtalk.io/document/orgapp/obtain-the-userid-of-a-user-by-using-the-log-free) API で unionid の値を取得できます。 - [ユーザー詳細を照会する](https://open.dingtalk.io/document/orgapp/query-user-details) API を呼び出して unionid の値を取得できます。       操作者に権限がない場合、API は `The operator has no permission` というエラーを返します。 |
...
本 API の呼び出しでエラーが発生した場合、エラーメッセージをもとに [グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1) ドキュメントから解決策を確認してください。
