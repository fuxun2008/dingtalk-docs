...
| 名前 | タイプ | 必須 | 例 | 説明 |
| --- | --- | --- | --- | --- |
| roleIds | String | はい | 1507113584 | ロールの roleId リスト。[ロールリストの取得](/zh/open/development/obtains-a-list-of-enterprise-roles)インターフェイスを呼び出して取得できます。  複数の roleId は半角カンマ（,）で区切り、最大 20 個まで指定できます。 |
| userIds | String | はい | user1 | 社員の userId。[電話番号によるユーザー検索](/zh/open/development/query-users-by-phone-number)を呼び出して取得できます。  複数の userId は半角カンマ（,）で区切り、最大 20 個まで指定できます。 |
...
当該インターフェイスの呼び出しでエラーが発生した場合は、エラーメッセージをもとに[グローバルエラーコード](/zh/open/development/server-api-error-codes-1)ドキュメントで解決策を検索してください。
