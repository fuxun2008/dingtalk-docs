...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| dentryUuid | String | はい | ファイル uuid。[ファイル検索](https://open.dingtalk.io/document/development/search-for-files)インターフェースまたは [dentryUuid 情報の取得](https://open.dingtalk.io/document/development/api-getuuidbydentryid)インターフェースを呼び出して、レスポンスパラメータの dentryUuid フィールドを取得できます。 |

### クエリパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| unionId | String | はい | ユーザー unionId。[ユーザー詳細の照会](https://open.dingtalk.io/document/development/query-user-details)インターフェースを呼び出して取得できます。 |
...
本インターフェースの呼び出し時にエラーが発生した場合、エラーメッセージに基づき [グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1) ドキュメントで解決方法を検索できます。
