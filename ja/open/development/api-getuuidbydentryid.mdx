---
title: "dentryUuid 情報の取得"
---

ファイルの spaceId と dentryId に基づいて、ファイルの dentryUuid 情報を取得します。
...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | 当該インターフェースを呼び出すためのアクセス認証情報。以下の方法で取得します：   - 社内アプリの場合、[社内アプリの accessToken を取得](https://open.dingtalk.io/document/development/obtain-the-access-token-of-an-internal-app) インターフェースを呼び出して取得します。 - サードパーティ社内アプリの場合、[サードパーティアプリの権限付与企業の accessToken を取得](https://open.dingtalk.io/document/development/obtain-the-access-token-of-the-authorized-enterprise-1) インターフェースを呼び出して取得します。 |

### パスパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| dentryId | String | はい | 現在のファイルの ID。 |

### クエリパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| spaceId | String | はい | スペース ID。 |
| operatorId | String | はい | 操作者の unionId。[ユーザー詳細の照会](https://open.dingtalk.io/document/development/query-user-details) インターフェースを呼び出して unionid パラメータ値を取得します。 |
...
| 403 | permissionDenied | The operator has no permission. | 現在のユーザーには当該操作の権限がありません |
| 404 | documentNotExist | %s | ドキュメントが存在しないか、フォーマットが未対応です |
