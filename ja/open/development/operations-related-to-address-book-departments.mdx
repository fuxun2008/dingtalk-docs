...
4. 連絡先関連の API を呼び出します。
   ...
   1. サーバー API の [部門の作成](/zh/open/development/address-book-creation-department-established-department) API を呼び出して部門を作成し、部門の `dept_id` を取得します。

      - ルート部門の子部門を作成する場合は、パラメータ parent_id に 1 を渡します。本サンプルではルート部門配下の子部門を作成します。
      - その他の部門の子部門を作成する場合は、先に [部門リストの取得](/zh/open/development/user-management-acquires-the-list-departments) API を呼び出し、取得した部門 ID を parent_id の値として使用する必要があります。
   2. 部門の `dept_id` に基づき、サーバー API の [部門詳細の取得](/zh/open/development/query-department-details0-v2) API を呼び出して、部門の詳細情報を取得します。
   3. 部門の `dept_id` に基づき、サーバー API の [部門の更新](/zh/open/development/address-book-update-department) API を呼び出して、部門情報を更新します。
   4. 部門の `dept_id` に基づき、サーバー API の [部門の削除](/zh/open/development/address-book-deletion-department) API を呼び出して、部門情報を削除します。
