...
前提条件：[アプリの作成と設定](/zh/open/dingstart/create-and-configure-an-application) のフローを完了してください。

ステップ 1：アプリ認証情報を取得し、クライアント ID と Client Secret を取得します。

ステップ 2：API 権限をリクエストし、スマート会議室関連の API 権限を申請します。

ステップ 3：アプリのアクセス認証情報を取得します。[社内アプリのアクセストークンを取得する](/zh/open/development/obtain-the-access-token-of-an-internal-app)。API を呼び出す際、アクセストークンによって呼び出し元の身元を認証します。

ステップ 4：会議室関連 API を呼び出します。

1. サーバー API [会議室グループの作成](/zh/open/development/create-meeting-room-groups) を呼び出し、会議室グループの戻り値 `result` フィールド、すなわち会議室グループ ID を取得します。
2. 会議室グループ ID に基づき、サーバー API [会議室グループ情報の更新](/zh/open/development/update-meeting-room-groups) を呼び出して、会議室グループ情報の更新操作を実行します。
3. サーバー API [会議室グループリストの照会](/zh/open/development/query-meeting-rooms-groups) を呼び出して、会議室グループリストのコンテンツを取得します。
4. 会議室グループ ID に基づき、サーバー API [会議室グループ情報の照会](/zh/open/development/query-meeting-room-groups) を呼び出して、個別の会議室グループの詳細コンテンツ情報を取得します。
5. 会議室グループ ID に基づき、サーバー API [会議室グループの削除](/zh/open/development/delete-a-conference-room-group) を呼び出して、会議室グループの削除操作を実行します。

## 前提条件

[アプリの作成と設定](/zh/open/dingstart/create-and-configure-an-application) のフローを完了してください。
...
ステップ 1 で取得したクライアント ID と Client Secret に基づき、アプリのアクセス認証情報を取得します。[社内アプリのアクセストークンを取得する](/zh/open/development/obtain-the-access-token-of-an-internal-app)。
