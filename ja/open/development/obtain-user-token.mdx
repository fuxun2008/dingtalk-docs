...
| code | String | いいえ | abcd | OAuth 2.0 の一時認可コード。サードパーティ社内アプリは統一認可スイート / [ログインユーザーのアクセス認証情報を取得](/zh/open/development/obtain-identity-credentials) を連携して、一時認可コード authCode を取得する必要があります。 |
...
本 API の呼び出しでエラーが発生した場合は、エラーメッセージを基に [エラーコード（旧版サーバーサイド）](/zh/open/development/server-api-error-codes-1) ドキュメントで解決方法を検索してください。
