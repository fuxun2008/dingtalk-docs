...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | 本インターフェース呼び出しのためのアクセス認証情報。以下の方法で取得します：   - 社内アプリの場合は、[社内アプリの accessToken を取得](https://open.dingtalk.io/document/development/obtain-the-access-token-of-an-internal-app#) インターフェースを呼び出して取得します。 - サードパーティ社内アプリの場合は、[サードパーティアプリで権限付与された企業の accessToken を取得](https://open.dingtalk.io/document/development/obtain-the-access-token-of-the-authorized-enterprise-1#) インターフェースを呼び出して取得します。 |

### クエリパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| unionId | String | はい | ファイルを受信するユーザーの unionId。[ユーザー詳細を照会](https://open.dingtalk.io/document/development/query-user-details#) インターフェースを呼び出して取得できます。 |

### リクエストボディ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| spaceId | String | はい | ファイルが存在するスペース ID。[スペースを追加](https://open.dingtalk.io/document/development/add-space#) インターフェースを呼び出して、id パラメータの値を取得します。 |
| dentryId | String | はい | ファイル ID。[ファイルまたはフォルダーリストを取得](https://open.dingtalk.io/document/development/obtain-the-file-list-storage#) インターフェースを呼び出して、id パラメータの値を取得できます。 |
...
本インターフェースの呼び出し時にエラーが発生した場合は、エラーメッセージに基づいて [グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1) ドキュメントで解決策を確認できます。
