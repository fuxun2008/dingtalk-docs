...
ステップ 1：[ファイル分割アップロードの初期化](/zh/open/development/initialize-a-multipart-upload-object) API を呼び出し、アップロード識別子 uploadKey を取得します。

ステップ 2：本 API を呼び出して、各ファイルのアップロード情報を取得します。

ステップ 3：各ファイルに対してアップロードを繰り返し実行します。

ステップ 4：ステップ 1 で取得したアップロード識別子 uploadKey を使用し、[ファイルの送信](/zh/open/development/submittal-file) API を呼び出してアップロードを完了します。
