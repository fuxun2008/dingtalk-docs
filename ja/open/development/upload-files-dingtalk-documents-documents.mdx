...
1. [マイドキュメントのナレッジベース情報を取得](/zh/open/development/get-my-documents)の API を呼び出し、「マイドキュメント」のルートノード ID（戻りパラメータ `rootNodeId`、すなわち parentDentryUuid）を取得します。
2. [ファイルアップロード情報を取得](/zh/open/development/obtain-file-upload-informations)の API を呼び出し、ファイルアップロードに必要な `resourceUrls` と `headers` のパラメータ値を取得します。
3. OSS の header リクエスト署名方式でファイルをアップロードします。
   ...
   ```
4. [ファイル送信](/zh/open/development/submittal-file)の API を呼び出し、ファイルを DingTalk ドキュメント（マイドキュメント）の目次にアップロードします。
