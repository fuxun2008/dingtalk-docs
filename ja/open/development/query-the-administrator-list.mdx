...
| 名前 | タイプ | 必須 | 例 | 説明 |
| --- | --- | --- | --- | --- |
| access_token | String | 必須 | bE74xxxx | 本 API を呼び出すアプリ認証情報。   - 社内アプリの場合、社内アプリの access_token 取得 API から取得します。 - サードパーティ社内アプリの場合、サードパーティ企業の access_token 取得 API から取得します。 |
...
本 API の呼び出しでエラーが発生した場合、エラーメッセージをもとに [グローバルエラーコード](/zh/open/development/server-api-error-codes-1) ドキュメントで解決方法を検索してください。
