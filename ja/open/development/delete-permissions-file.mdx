---
title: "権限の削除"
description: "ナレッジベース、ドライブファイルのメンバー権限を削除し、協業範囲を制御します"
---
...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | 本インターフェース呼び出しのアクセス認証情報です。[社内アプリの access_token を取得](/ja/open/development/obtain-the-access-token-of-an-internal-app)インターフェースを呼び出して取得できます。 |

### パスパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| dentryUuid | String | はい | ファイル UUID。[ファイル検索](/ja/open/development/search-for-files)インターフェースまたは [dentryUuid 情報の取得](/ja/open/development/api-getuuidbydentryid)インターフェースを呼び出し、レスポンスパラメータの dentryUuid フィールドを取得できます。 |
