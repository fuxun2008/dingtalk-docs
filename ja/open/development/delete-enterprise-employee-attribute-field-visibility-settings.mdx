---
title: "ユーザー属性の表示設定を削除する"
---

本 API を呼び出して、企業の社員属性フィールドの表示設定を削除します。

## API 呼び出し説明

ユーザー属性の表示設定 API を呼び出した後、本 API を呼び出すことで、企業の社員属性フィールドに対する非表示設定情報を削除できます。
...
| 必要な権限 | permission-Contact.ReachableRule.ReadWrite-連絡先データの表示範囲管理権限 |

### リクエストヘッダー

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | この API を呼び出す際のアクセス認証情報。以下の方法で取得します。   - 社内アプリの場合は、[社内アプリの access_token を取得する](https://open.dingtalk.io/document/development/obtain-the-access-token-of-an-internal-app#) API を呼び出して取得します。 - サードパーティ社内アプリの場合は、[サードパーティアプリで権限付与された企業の access_token を取得する](https://open.dingtalk.io/document/development/obtain-the-access-token-of-the-authorized-enterprise-1#) API を呼び出して取得します。 |

### パスパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| settingId | Long | はい | 設定の ID。[ユーザー属性の表示設定](https://open.dingtalk.io/document/development/add-or-update-the-hidden-settings-of-the-employee-property#) API を呼び出すことで id パラメータの値を取得できます。 |
...
| 名前 | タイプ | 説明 |
| --- | --- | --- |
| result | Boolean | 削除に成功したかどうか。true は成功を示します。 |
...
| --- | --- | --- | --- |
| 400 | parameter.error | パラメータエラー | settingId パラメータエラー |
