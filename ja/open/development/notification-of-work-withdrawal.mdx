...
| msg_task_id | Number | 必須 | 256271667526 | チャット送信時に DingTalk から返却されるタスク ID。[業務通知の送信](/zh/open/development/asynchronous-sending-of-enterprise-session-messages) API を呼び出して task_id パラメータの値を取得します。  **説明**  送信から 24 時間以内の業務通知のみ送信取消できます。 |
...
本 API の呼び出しでエラーが発生した場合、[グローバルエラーコード](/zh/open/development/server-api-error-codes-1)ドキュメントでエラーメッセージから解決方法を検索できます。
