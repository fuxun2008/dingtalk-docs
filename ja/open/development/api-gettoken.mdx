...
description: "社内アプリとサードパーティ社内アプリで企業 API を呼び出すための認証情報を取得します"
---
...
- 社内アプリ：[社内アプリの accessToken を取得する](/ja/open/development/obtain-the-access-token-of-an-internal-app#) API を呼び出して、企業が API を呼び出すための認証情報を取得していました。
- サードパーティ社内アプリ：[サードパーティアプリで権限付与された企業の accessToken を取得する](https://open.dingtalk.com/document/development/obtain-the-access-token-of-the-authorized-enterprise-1#) API を呼び出して、サードパーティアプリで権限付与された企業が API を呼び出すための認証情報を取得していました。
...
API 呼び出し時にエラーが発生した場合は、エラーメッセージをもとに [グローバルエラーコード](/ja/open/development/server-api-error-codes-1#) ドキュメントで解決方法を検索してください。
