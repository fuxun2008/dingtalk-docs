...
description: "ユーザーのマイドキュメント内のナレッジベースワークスペース一覧を取得します"
---
...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | 当該 API を呼び出すためのアクセス認証情報です。次の方法で取得します:   - 社内アプリの場合は、[社内アプリの access_token を取得](/ja/open/development/obtain-the-access-token-of-an-internal-app#)を呼び出して取得します。 - サードパーティ社内アプリの場合は、[サードパーティアプリが権限付与された企業の access_token を取得](https://open.dingtalk.com/document/development/obtain-the-access-token-of-the-authorized-enterprise-1#)を呼び出して取得します。 |
...
当該 API の呼び出しでエラーが発生した場合、エラーメッセージをもとに[グローバルエラーコード](/ja/open/development/server-api-error-codes-1)ドキュメントで解決方法を検索してください。
