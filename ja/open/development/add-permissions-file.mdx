...
本インターフェースを呼び出し、ファイル uuid、操作者 unionId および権限 id などの情報に基づいて、権限ユーザーへの権限追加を実現します。
...
| 必要な権限 | permission-Storage.Permission.Write-企業ストレージ権限付与情報書き込み権限 |

### リクエストヘッダー

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | はい | 本インターフェース呼び出しのアクセス認証情報。[社内アプリのアクセストークンを取得](https://open.dingtalk.io/document/development/obtain-the-access-token-of-an-internal-app)インターフェースを呼び出して取得します。 |

### パスパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| dentryUuid | String | はい | ファイル uuid：[ファイル検索](https://open.dingtalk.io/document/development/search-for-files)インターフェース、または [dentryUuid 情報の取得](https://open.dingtalk.io/document/development/api-getuuidbydentryid)インターフェースを呼び出し、レスポンスパラメータ `dentryUuid` フィールドを取得できます。 |

### クエリパラメータ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| unionId | String | はい | ユーザー unionId。[ユーザー詳細の照会](https://open.dingtalk.io/document/development/query-user-details)インターフェースを呼び出して unionid パラメータ値を取得できます。 |

### リクエストボディ

| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| roleId | String | はい | ロール id、列挙値：   - **OWNER**：所有者。以下の権限スコープを持ちます：    - PermissionPrivilegeEnum.INFO   - PermissionPrivilegeEnum.LIST   - PermissionPrivilegeEnum.PREVIEW   - PermissionPrivilegeEnum.READ   - PermissionPrivilegeEnum.WRITE   - PermissionPrivilegeEnum.DOWNLOAD   - PermissionPrivilegeEnum.ADD   - PermissionPrivilegeEnum.DELETE   - PermissionPrivilegeEnum.MODIFY   - PermissionPrivilegeEnum.COPY   - PermissionPrivilegeEnum.RENAME   - PermissionPrivilegeEnum.READ_PERMISSION   - PermissionPrivilegeEnum.WRITE_PERMISSION   - PermissionPrivilegeEnum.ASSIGN - **MANAGER**：管理者。以下の権限スコープを持ちます：    - PermissionPrivilegeEnum.INFO   - PermissionPrivilegeEnum.LIST   - PermissionPrivilegeEnum.PREVIEW   - PermissionPrivilegeEnum.READ   - PermissionPrivilegeEnum.WRITE   - PermissionPrivilegeEnum.DOWNLOAD   - PermissionPrivilegeEnum.ADD   - PermissionPrivilegeEnum.DELETE   - PermissionPrivilegeEnum.MODIFY   - PermissionPrivilegeEnum.COPY   - PermissionPrivilegeEnum.RENAME   - PermissionPrivilegeEnum.READ_PERMISSION   - PermissionPrivilegeEnum.WRITE_PERMISSION - **EDITOR**：編集者。以下の権限スコープを持ちます：    - PermissionPrivilegeEnum.INFO   - PermissionPrivilegeEnum.LIST   - PermissionPrivilegeEnum.PREVIEW   - PermissionPrivilegeEnum.READ   - PermissionPrivilegeEnum.WRITE   - PermissionPrivilegeEnum.DOWNLOAD   - PermissionPrivilegeEnum.ADD - **DOWNLOADER**：閲覧・ダウンロード者。以下の権限スコープを持ちます：    - PermissionPrivilegeEnum.INFO   - PermissionPrivilegeEnum.LIST   - PermissionPrivilegeEnum.PREVIEW   - PermissionPrivilegeEnum.READ   - PermissionPrivilegeEnum.DOWNLOAD - **READER**：閲覧のみ可能。以下の権限スコープを持ちます：    - PermissionPrivilegeEnum.INFO   - PermissionPrivilegeEnum.LIST   - PermissionPrivilegeEnum.PREVIEW |
| members | Array | はい | 権限ユーザーリスト。最大 30 件。 |
| type | String | はい | 権限ユーザーのタイプ、列挙値：   - **ORG**：企業 - **DEPT**：部門 - **TAG**：カスタム tag - **CONVERSATION**：チャット - **USER**：ユーザー |
| id | String | はい | 権限ユーザーの id：   - `type=ORG` の場合、id は組織ID - `type=DEPT` の場合、id は部門 ID - `type=TAG` の場合、id はタグ ID - `type=CONVERSATION` の場合、id はチャット ID - `type=USER` の場合、id は社員 userId |
| corpId | String | いいえ | 権限の帰属先企業：       - 企業 id が存在する場合、対応する member が退職した際に権限が自動的にクリアされます。 - memberType が dept タイプの場合、必ず企業 id が必要です。 |
| name | String | いいえ | ユーザー名：   - `type=ORG` の場合、name は企業名 - `type=DEPT` の場合、name は部門名 - `type=TAG` の場合、name はタグ名 - `type=CONVERSATION` の場合、name はチャット名 - `type=USER` の場合、name は社員名前 |
| option | Object | いいえ | オプションパラメータ。 |
| duration | Long | いいえ | 有効時間（秒）。       - 現在は OwnerType が APP の Space のみ一時的な権限をサポートします。 - 最大値は 3600。 |
...
本インターフェース呼び出し時にエラーが発生した場合は、エラーメッセージに基づいて[グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1)ドキュメントから解決策を参照できます。
...
| 403 | permissionDenied | %s | ユーザーに付与権限が不足しています |
