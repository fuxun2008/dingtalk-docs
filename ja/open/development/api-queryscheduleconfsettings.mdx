...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| scheduleConferenceId | String | はい | 予約会議 ID。[予約会議を作成](https://open.dingtalk.io/document/development/create-appointment-meeting#) API のレスポンスパラメータ `scheduleConferenceId` フィールドから取得できます。 |
...
この API の呼び出し時にエラーが発生した場合は、エラーメッセージに基づき[グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1)ドキュメントから解決方法を検索してください。
