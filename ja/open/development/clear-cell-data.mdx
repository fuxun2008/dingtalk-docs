...
| 名前 | タイプ | 必須 | 説明 |
| --- | --- | --- | --- |
| workbookId | String | はい | スプレッドシートファイルの ID。[ナレッジベース API](https://open.dingtalk.io/document/development/knowledge-base-overview#) が返す `nodeId(dentryUuid)` がスプレッドシートの `workbookId` に該当します。[ノードを取得](https://open.dingtalk.io/document/development/get-node#) および [ナレッジベースドキュメントを作成](https://open.dingtalk.io/document/development/create-team-space-document#) API を呼び出して取得できます。 |
| sheetId | String | はい | ワークシートの ID または見出し。   - 社内アプリの場合、[全ワークシートを取得](https://open.dingtalk.io/document/development/obtain-all-worksheets#) API を呼び出して id を取得するか、ワークシートの見出しを渡します。 - サードパーティ社内アプリの場合、ワークシートの見出しを渡すことを推奨します。   ワークシートの見出しの確認手順は下図のとおりです。 |
...
本 API の呼び出し時にエラーが発生した場合は、エラーメッセージをもとに[グローバルエラーコード](https://open.dingtalk.io/document/development/server-api-error-codes-1)ドキュメントで解決策を検索してください。
