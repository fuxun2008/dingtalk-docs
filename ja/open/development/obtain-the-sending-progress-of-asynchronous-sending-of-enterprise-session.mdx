...
| task_id | Number | はい | 256271667526 | チャット送信時に DingTalk から返されるタスク ID。[業務通知を送信する](/zh/open/development/asynchronous-sending-of-enterprise-session-messages) API を呼び出して task_id パラメータの値を取得します。  **説明**  過去 24 時間以内のタスクのみクエリ可能です。 |
...
この API の呼び出しでエラーが発生した場合は、エラーメッセージをもとに [グローバルエラーコード](/zh/open/development/server-api-error-codes-1) ドキュメントから解決策を検索してください。
