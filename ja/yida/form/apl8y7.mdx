---
title: "カスタムコンポーネント例 - アプリ一覧"
description: "Yidaでアプリ一覧を表示するカスタムコンポーネントの作成手順を解説します。活用シーン、動作イメージ、コンポーネントの作成、スキーマインポートの有効化など、具体的な設定手順を詳しく紹介します。"
polished: true
---
...
- 本コンポーネントは、属性設定やマルチデバイス対応の参考としてご活用ください。
- 本コンポーネントは学習目的でのみ提供しており、プラットフォームが公式に提供する標準コンポーネントではありません。
</Note>

## 1. 活用シーン

**アプリ一覧**機能は、主に表示ページで利用されます。
...
##### 2. スキーマインポート機能を有効化します。手順はYidaのフォームデザイナーと同様です。詳細は以下を参照してください：

- コンポーネントデザイナーを開き、ブラウザのコンソールを起動して、以下のコードを実行します。

```javascript
document.querySelector('.lc-left-area-bottom div:last-child').style.display = 'block'
```

##### 3. 以下のスキーマをコンポーネントデザイナーのスキーマコンソールに貼り付け、「スキーマインポート」をクリックします。
        ...
        "__propTypes__": [
          {
            ...
            "__batchEditFormatToLines__": {
              ...
              "name": "urlParams",
              "description": "現在のページURLに含まれるパラメータ。例：aliwork.com/APP_xxxx/workbench?id=1&name=Yida の場合、this.state.urlParams.name で Yida を取得できます。",
