#!/usr/bin/env python3
"""
批量翻译 mdx：<root>/*.mdx (en 母版) → ms/<root>/*.mdx (马来文 Bahasa Melayu)。

克隆自 translate_mdx_en2id.py（en→x 翻译链），仅改：目标 id/→ms/、词库 zh-id→zh-ms、
prompt 资产译马来语、输出目录 id→ms、claude CLI 二进制走 CLAUDE_CLI_BIN 环境变量。

设计要点（与 translate_mdx_batch.py 的区别，同 en2id）：
- 源根 = 仓库根 <root>/（英文母版），目标 = ms/<root>/。
- **保留图片 / 视频 / iframe / Frame 及其 URL 原样**（不剥离）——ms 与 en 共享同一批远程图。
- en→ms 词库：由 zh-en.json 与 zh-ms.json 经共享 zh key 合成 en→ms 对照表（强约束）。
- frontmatter description 走 **SEO 优化**：生成面向搜索的马来语摘要，而非首段直译。

实现方式：通过 `<CLAUDE_CLI_BIN> -p --bare` 子进程调用（默认二进制 cc_222，规避集团云壳 SIGKILL）。

CLI:
  bash scripts/run_translate_ms.sh --root meetings --concurrency 1
  bash scripts/run_translate_ms.sh --root im                       # 跳过已存在的非占位译文，只补缺失
  bash scripts/run_translate_ms.sh --root drive --only drive/index
  bash scripts/run_translate_ms.sh --root contacts --dry-run --limit 2
  bash scripts/run_translate_ms.sh --root open --bucket-count 4 --bucket-id 0
"""
...
# 原生 claude CLI 会被集团云壳 SIGKILL（exit 137），必须走独立二进制（cc_222）。
...
SYSTEM_RULES = """你是钉钉国际版帮助中心的资深马来语技术文档翻译。你的任务是把英文母版 mdx 译为马来文 (Bahasa Melayu)，作为官方文档面向马来西亚企业用户，必须达到母语级流畅 + 商务专业 (bahasa baku)。
...
3. 代码块 ``` 内部按 4 类区分处理（严禁一刀切「全部不动」）：
   ...
   3b. 示例字面量（**必须译为马来语**）：示例 JSON 中 user-facing 字段（title/content/text/key/value/description/desc/name 等）的字符串值；表格 example 列的示例值。
       例：`"content": "Monthly meeting notification"` → `"content": "Notifikasi mesyuarat bulanan"`
       例：`"key": "Name"` / `"value": "John"` → `"key": "Nama"` / `"value": "Ahmad"`
   3c. 代码注释 + markdown 语法示例（**必须翻译**）：` // ... ` ` # ... ` 行内注释；```text``` 块内的纯 markdown 语法说明。
   3d. API enum 必传中文值（**保留原值 + 加马来语行内注释**）：例 `"sticker": "灯泡"  // Mentol`。
   代码块 title="x" 的 x 不译；``` 后的 lines / json lines 等修饰符不动。
4. 【图片/媒体原样保留】图片 ![alt](url)、<img/>、<video>...</video>、<iframe>...</iframe>、<Frame>...</Frame> 及其 URL **完全保持原样**（不删不改）。仅把 alt / caption 文本译为马来语。图片 URL 必须与英文版完全一致。
...
9. 标点用拉丁习惯：. , ? ! " ( )（同英文）；马来语千分位用「,」小数用「.」（1,000.50）；日期 2 Jun 2026 格式。
...
description 不要是首段直译或截断，而要生成面向搜索的马来语摘要：
- 一句话清楚传达页面价值
- 自然织入核心关键词（产品名 / 功能名 / 用户意图词，如 DingTalk / AI Table / automasi / tetapan kebenaran / perkongsian）
- 长度约 100–160 字符（避免搜索结果被截断）
- 不是关键词堆砌，而是自然可读的一句话
- 含一个明确的动作或收益
- 不只是 title 的改写（补充信息）
例：title「Menambah ahli」→ description「Ketahui cara menambah dan mengurus ahli organisasi di DingTalk secara pukal — daripada pendaftaran kenalan hingga penetapan jabatan, panduan untuk pentadbir menguruskan ahli dengan cekap.」"""

STYLE_MS = """Panduan gaya (Bahasa Melayu):
- 目标读者：马来西亚企业用户与 IT 管理员（pengguna perniagaan & pentadbir IT）
- 语气：jelas、profesional、动作导向（imperatif：Klik X / Pilih X / Buka X，不写 Anda boleh klik）；FAQ 可适度口语但保持 bahasa baku
- 句式：短句优先；避免被动堆砌；一句话讲一件事
- Heading：Sentence case（仅首字母 + 专有名词大写），避免 Title Case
- 用词：通用 IT 术语用马来语主流译法 — Tetapan / Ciri / Kebenaran / Fail / Folder / Muat turun / Muat naik / Log masuk / Log keluar / Urus / Kongsi / Padam / Simpan / Hantar
...
- 功能更新条目：Baharu / Dipertingkatkan / Dibaiki / Tidak lagi digunakan
- 注意：避免英式直译腔、冗长被动语态；追求自然马来语（Bahasa Melayu baku，非印尼语——用 anda 非 kamu、boleh 非 bisa、fail 非 berkas、jemput 非 undang 视语境）"""

# 开放平台 (Open Platform / OpenAPI) 专属规则 — 仅 root=open 注入
...
C. 「企业内部应用」→ Aplikasi dalaman；「第三方应用」→ Aplikasi pihak ketiga；「服务端 API」→ Server API。
...
【Heading / 语气】
I. 马来文 Heading 用 Sentence case（仅首字母 + 专有名词大写，不模仿英文源文的 Title Case）；动作导向祈使句（Klik / Panggil / Konfigurasikan）。"""
...
【表格列头 / 用词】
H. Parameter→Parameter / Description→Penerangan / Required→Wajib / Example→Contoh / Notes→Nota；
   Feature→Ciri / Supported→Disokong / Not supported→Tidak disokong。
I. 套餐：Free→Edisi Percuma / Basic→Edisi Basic / Professional→Edisi Professional / Dedicated→Edisi Dedicated。"""
...
def load_en2ms_glossary() -> dict[str, str]:
    """zh-en.json 与 zh-ms.json 经共享 zh key 合成 en→ms 对照表。

    同一 en 命中多个不同 ms 的歧义对会造成错误强约束，因此**整对丢弃**，
    只保留明确 1:1 对（高精度优先）。
    """
    ...
    with (GLOSSARY_DIR / "zh-en.json").open(encoding="utf-8") as f:
        zh_en = json.load(f)
    with (GLOSSARY_DIR / "zh-ms.json").open(encoding="utf-8") as f:
        zh_ms = json.load(f)
    ...
    for zh, en_val in zh_en.items():
        ...
        ms_clean = ms_val.strip().strip("`").strip()
        if not en_key or not ms_clean:
            continue
        candidates[en_key].add(ms_clean)

    return {en: next(iter(ms)) for en, ms in candidates.items() if len(ms) == 1}
...
def build_user_message(hits: dict[str, str], source: str) -> str:
    if hits:
        ...
        return (
            "本篇命中的项目术语 en→ms 对照表（强约束 — 必须严格按此译，不得换同义词）：\n"
            f"```json\n{terms_json}\n```\n\n"
            "凡未在表中出现的术语，按你自己的判断译，保持同等专业度。\n\n"
            "---\n\n"
            "下面是英文原文 mdx。按上面所有铁律、SEO 规则、风格指南译为马来文（直接输出译文 mdx，不要任何前后缀）：\n\n"
            f"{source}"
        )
    return (
        "下面是英文原文 mdx。按所有铁律、SEO 规则、风格指南译为马来文（直接输出译文 mdx，不要任何前后缀）：\n\n"
        f"{source}"
    )


def build_system_prompt(root: str = "") -> str:
    sections = [SYSTEM_RULES, SEO_DESCRIPTION_RULE, STYLE_MS]
    if root == "open" or root.startswith("open/"):
        sections.append(OPEN_PLATFORM_RULES)
    if "yida" in root.split("/"):
        sections.append(YIDA_RULES)
    sections.append("本次任务把英文 mdx 译为马来文（Bahasa Melayu，商务书面语 bahasa baku）。直接输出译文 mdx，不加任何前后缀说明。")
    return "\n\n".join(sections)
...
async def call_claude_cli(system_prompt: str, user_msg: str, model: str, timeout_s: int) -> tuple[str, dict]:
    ...
    # 二进制走 CLAUDE_CLI_BIN（默认 cc_222）——原生 claude 会被集团云壳 SIGKILL。
    proc = await asyncio.create_subprocess_exec(
        CLAUDE_CLI_BIN, "-p", "--bare",
        "--model", model,
...
def gather_tasks(
    ...
    # 特例：root == "." 只译仓库根的 index.mdx（Overview 首页）→ ms/index.mdx
    if root in (".", ""):
        src = REPO_ROOT / "index.mdx"
        if not src.exists():
            sys.exit(f"ERROR: root index.mdx not found: {src}")
        return [FileTask(source=src, target=REPO_ROOT / "ms" / "index.mdx", rel="index.mdx")]
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    print(
        f"[info] EN→MS root={args.root} 任务数={len(tasks)} only={args.only} "
        f"concurrency={args.concurrency} model={model} glossary_en2ms={len(glossary)} bin={CLAUDE_CLI_BIN}"
    )
    if args.dry_run:
    ...
    write_report(results, OUTPUT_DIR / "ms", started, ended, suffix)

    failed = [r for r in results if r.status == "failed"]
    print(f"\n[done] report: {OUTPUT_DIR / 'ms' / f'report{suffix}.md'}")
    return 1 if failed else 0


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="批量翻译 mdx：<root>/ (en 母版) → ms/<root>/ (马来文，保留图片)")
