---
title: "宜搭平台接口"
description: "本文档介绍了如何使用宜搭平台进行流程实例的发起，包括调用说明、接口地址拼接规则以及参数配置等详细步骤。"
---
...
<Frame>
  ![](https://yida-support.oss-cn-shanghai.aliyuncs.com/static/png/1628077163254-8aa950f8-3c6e-4a93-8354-d2b3feb0903d.png)
</Frame>

请参考【二、流程实例】下面的【1. 流程发起】里面有详细的实战演示案例

**注意，目前版本的searchFieldJson和dynamicOrder字段需要的是一个字符串类型的值。比如：**
...
| formDataJson | 表单数据 | 是 | <br /> | 类型：String<br />参考：附录1保存/更新 表单数据格式说明 |
...
<Frame>
  ![](https://yida-support.oss-cn-shanghai.aliyuncs.com/static/png/1615876628810-e94d85cd-d293-4b5f-9c87-f6db0514f95f.png)
</Frame>

②. 新建一个高级展示页，拖动一个 Button 按钮到页面中，添加一个数据源

<Frame>
  ![](https://yida-support.oss-cn-shanghai.aliyuncs.com/static/png/1619403063690-dbab2233-6c3d-45b2-9066-8cbf9aacbcd1.png)
</Frame>
...
<Frame>
  ![](https://yida-support.oss-cn-shanghai.aliyuncs.com/static/png/1615876757818-a5877732-b88e-4922-abdd-de96a9aebbf5.png)
</Frame>

③. 为按钮添加一个 onClick 事件，点击 \<> 图标前往进行远程数据源与按钮方案绑定的实现：

<Frame>
  ![](https://yida-support.oss-cn-shanghai.aliyuncs.com/static/png/1615876929012-2222d08c-00e1-43b1-a808-d1e3a09495a4.png)
</Frame>

参考：[https://developers.aliwork.com/docs/api/about](/zh/open/yida/api/about)
...
<Frame>
  ![](https://yida-support.oss-cn-shanghai.aliyuncs.com/static/png/1615877395212-3f8ca90e-ffc5-4103-8a76-4c766a3ce98a.png)
</Frame>
...
<Frame>
  ![](https://yida-support.oss-cn-shanghai.aliyuncs.com/static/png/1615877011510-34910163-592b-4182-bc2a-419616a78a2f.png)
</Frame>
...
<Frame>
  ![](https://yida-support.oss-cn-shanghai.aliyuncs.com/static/png/1615877114726-b62541ba-f2ad-4b1b-8093-ef31db669b8f.png)
</Frame>

⑥. 接下来，我们再次前往对应的流程的数据管理页面，找到了我们新发起的那一条流程，并且单行文本中填写的内容就是我们在程序中填写的固定值 123，全部测试验证完成。

<Frame>
  ![](https://yida-support.oss-cn-shanghai.aliyuncs.com/static/png/1615877255394-c4377909-7356-4a54-bca6-fcec9175315a.png)
</Frame>

- 视频展示

<video controls preload="metadata" poster="https://img.alicdn.com/imgextra/i2/6000000004280/O1CN01jalg5i1hUHqGc55A4_!!6000000004280-0-tbvideo.jpg" src="https://yida-support.oss-cn-shanghai.aliyuncs.com/static/mp4/311018400839.mp4"></video>
...
| searchFieldJson | 根据表单内组件值查询 | 否 | <br /> | 类型：String<br />格式见附录2：根据组件值进行条件搜索，组件值格式说明 |
...
| createTo | createFrom和createTo两个时间构造一个时间段。查询在该时间段创建的数据列表。 | 否 | 2018-02-01 | 字符串格式，且为yyyy-MM-DD格式。<br />和createFrom一起，相当于查询在<br />2018-01-01到2018-01-31之间(包含01和31号)创建的数据。<br />yyyy-MM-DD后补充0时0分0秒获取数据 |
| modifiedFrom | modifiedFrom和modifiedTo构成一个时间段，查询在该时间段有修改的数据列表 | 否 | 2018-01-01 | 字符串格式，且为yyyy-MM-DD格式 |
| modifiedTo | modifiedFrom和modifiedTo构成一个时间段，查询在该时间段有修改的数据列表。 | 否 | 2018-02-01 | 字符串格式，且为yyyy-MM-DD格式。 和modifiedFrom一起，相当于查询在 2018-01-01到2018-01-31之间(包含01和31号)被修改的数据。 |
...
| 参数名 | 描述 | 是否必填 | 示例 | 备注 |
|---|---|---|---|---|
| processInstanceId | 流程实例ID | 是 | f30233fb-72e1-4af4-8cb8-c7e0ea9ee530 | <br /> |
...
| 参数名 | 描述 | 是否必填 | 示例 | 备注 |
|---|---|---|---|---|
| processInstanceIds | 流程实例ID列表，多个用,分割 | 是 | f30233fb-72e1-4af4-8cb8-c7e0ea9ee530,d230233fb-72e1-4af4-8cb8-c7e0ea9ee530 | <br /> |
...
| formDataJson | 更新的表单值 | 否 | <br /> | 参考：附录1保存/更新 表单数据格式说明。<br />参数有的组件更新，没有的组件保持不变。<br />明细的值只能统一更新，无法只更新子表单下某个组件的值 |
...
| 参数名 | 描述 | 是否必填 | 示例 | 备注 |
|---|---|---|---|---|
| processInstanceId | 实例ID | 是 | <br /> | <br /> |
...
| appType | 应用ID | 是 | APP_DR4OK27ZKL5N22B907E8 | <br /> |
| formDataJson | 表单数据 | 是 | \{"textField_jcpm6agt": "单行","employeeField_jcos0sar": ["workno"]} | 类型：String<br />参考：[附录1保存/更新 表单数据格式说明](/zh/yida/developer-features/aql605) |
...
| 参数名 | 描述 | 是否必填 | 示例 | 备注 |
|---|---|---|---|---|
| formInstId | 要更新的表单数据ID | 是 | FINST-NJYJZELVVYZRVGJHR7M6FJW3ESJN1P1TCNPCJ9 | <br /> |
| updateFormDataJson | 要更新的表单组件值，必填 | 是 | \{"employeeField_jcpm5gy2":<br />["xxxxx","yyyyy"]}<br />（以成员组件示例） | 类型：String<br />参考：[附录1保存/更新 表单数据格式说明。](/zh/yida/developer-features/aql605) 参数有的组件更新，没有的组件保持不变。 明细的值只能统一更新，无法只更新子表单下某个组件的值 |
| useLatestVersion | 使用最新的表单版本进行更新 | 否 | y | 【特别注意】<br />默认值为n，建议使用y。 |
...
| 参数名 | 描述 | 是否必填 | 示例 | 备注 |
|---|---|---|---|---|
| formInstId | 要删除的表单数据ID | 是 | FINST-NJYJZELVVYZRVGJHR7M6FJW3ESJN1P1TCNPCJ9 | <br /> |
...
| 参数名 | 描述 | 是否必填 | 示例 | 备注 |
|---|---|---|---|---|
| formInstId | 要查询的实例的实例ID | 是 | FINST-NJYJZELVVYZRVGJHR7M6FJW3ESJN1P1TCNPCJ9 | <br /> |
...
| 附件组件 | 字符串数组 | [<br />\{<br />"downloadUrl": "文件下载地址",<br />"name": "文件名",<br />"previewUrl": "文件预览地址",<br />"url": "文件下载地址",<br />"ext": "docx"<br />}<br />] | <br /> |
| 图片组件 | 字符串数组 | [<br />\{<br />"downloadUrl": "文件下载地址",<br />"name": "文件名",<br />"previewUrl": "文件预览地址",<br />"url": "文件下载地址",<br />}<br />] |  |
| 超链接组件 | 字符串数组 | [<br />\{<br />"link":"http://www.yidaapps.com",<br />"text":"宜搭"<br />}<br />] | <br /> |
| 子表单 | JSONARRAY | [\{"textField_jcr0069m": "danhang1"},\{"textField_jcr0069m": "danhang2"}]<br />(textField_jcr0069m为子表单下单行的组件ID) | 由于子表单下有多条记录，所以用JSONARRAY。由于每条记录都是很多组件的值，因此用JSONObject来存每个组件对应的值 |
| 手写签名 | 字符串 | "图片地址" | <br /> |
...
| 城市选择 | 字符串数组 | [500000, 500100, 500104, 500104003] | 数组搜索。<br />搜索值必须是城市值的子集。<br />另外，有市ID，就必须有省ID。有区ID，就必须有省ID和市ID。 |
| 部门选择 | 数字 | 1123456 | 精确匹配 |
| 级联选择 | 字符串数组 | ["part","part_b"] | 数组搜索。和城市选择限制条件一致。 |
| 子表单组件 | 字符串 | danhang | 模糊搜索。<br />子表单下的值为一个大text，搜索用模糊搜索。 |
...
| formInstId | 实例ID | FINST-EF6Y93URN2F02S745LTMW2D2G4WVDS16O17ISCJ0 | <br /> |
