---
title: "编写更多语言 SDK"
description: "本页面介绍了如何通过HTTP请求与API网关进行交互，包括构成HTTP Request URL、设置必要的HTTP Request Header以及签名计算方法等详细说明。同时提供了HTTP Response内容结构和常见问题解答。"
---
...
- 其中：operation 业务开发配置暴露的api name，大小写敏感。 api url要求必须是https
...
- X-Hmac-Auth-IP 用户的IP地址
- X-Hmac-Auth-MAC 用户的MAC地址

- X-Hmac-Auth-Timestamp 请求发出时间，使用UTC时间，ISO8601格式为"yyyy-MM-ddTHH:mm:ss.sss+08:00"
- X-Hmac-Auth-Version API的版本号，版本号目前值为1.0

- X-Hmac-Auth-Nonce API的nonce，建议使用13位时间毫秒数 + 4位随机数
- apiKey 应用的 appkey
...
- 如果出现tengine返回的400错误，提示：Your browser sent a request that this server could not understand. 请检查先你的pom依赖，是不是有低版本的commons-codec，请exclude掉。 原因是由于低版本的commons-codec的base64出来的字符串多了 导致http报错格式错误。
- 签名报错，返回：SignatureDoesNotMatch 请检查下自己的apiKey是否正确 如果非java的客户端，检查下自己的参数是否按照升序排序 如果参数是中文，签名时，中文参数不要encode，传参数时要url encode
