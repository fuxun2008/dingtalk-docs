---
title: "数据源面板"
description: "本文档介绍了如何在不同版本中新建数据源，包括变量、远程请求和参数数据源的创建方法。同时详细解释了远程请求的数据格式要求、自动加载选项、加载方式选择、请求条件设置以及willFetch, fit, didFetch, onError等函数的具体用法。"
---
...
<Frame>
  ![](https://yida-support.oss-cn-shanghai.aliyuncs.com/static/png/1612072981152-bd94b719-798e-4eb5-b376-9bd6cace3b22.png)
</Frame>
...
<Frame>
  ![](https://yida-support.oss-cn-shanghai.aliyuncs.com/static/png/1612072981158-2f348b0e-920d-4413-ba9f-ad4cfd58fe9d.png)
</Frame>
...
<Frame>
  ![](https://yida-support.oss-cn-shanghai.aliyuncs.com/static/png/1612072981152-f7e8a57b-25d5-4098-a2b6-7215e2718ac5.png)
</Frame>
...
- 入参格式不正确。
- 您的接口支持 www.yidaapps.com 的跨域访问。
- 您的接口是 https，并且证书是「安全」的。

### 3.2 排查思路

1. 判断数据源的加载方式是 手动加载 还是 自动加载 ，如为 手动加载 ，请在数据源加载代码处 打断点 便于调试。
2. 预览/访问页面。
3. 打开浏览器控制台。
4. 触发数据源加载。
5. 查看控制台内对应请求信息进行排错。
...
<Frame>
  ![](https://yida-support.oss-cn-shanghai.aliyuncs.com/static/png/1646290600967-8e642d2d-3dea-4fb1-9fb8-f60b50e545da.png)
</Frame>

### 3.3 调用宜搭平台接口时，报错307？

请检查你的组织是否开启了二级域名，若开启二级域名应该将接口访问地址由`www.yidaapps.com/...`更换为相对地址`/dingtalk/web/APP_X1X2X3X4/v1/process/startInstance.json`即可避免该错误的出现。
