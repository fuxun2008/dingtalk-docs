...
- 在你的业务对数据的实时性要求较高时。例如：在新员工入职或者离职时，应用需要第一时间变更用户数据，此时就可以订阅[通讯录事件](https://open.dingtalk.com/document/app/address-book-events#topic-1950634)。
...
3. 成功配置请求地址后，在**事件****订阅**列表区域，开启要订阅的事件。
