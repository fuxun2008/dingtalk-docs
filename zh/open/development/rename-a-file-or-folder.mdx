---
title: "重命名文件或文件夹"
description: "调用本接口可对钉钉云盘中的文件或文件夹进行重命名，但存储空间的根目录不支持重命名。"
---
...
| dentryId | String | 是 | 文件或文件夹Id，可调用[获取文件或文件夹列表](/zh/open/development/get-a-list-of-files-or-folders)或[根据dentryUuid获取spaceId和dentryId](/zh/open/development/api-getdentryidbyuuid)接口获取id参数值。  存储空间根目录不支持重命名，因此该参数不支持传0。 |
