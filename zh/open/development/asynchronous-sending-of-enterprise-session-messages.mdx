...
- 不适用场景：需要发送一条任务类的通知提醒给员工，比如审批任务等，调用此接口无法实现。这种场景可以使用[创建待办](/document/development/add-dingtalk-to-do-task#)接口。
...
| --- | --- | --- | --- | --- |
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| agent\_id | Long | 是 | 123L | 发送消息时使用的微应用的AgentID。   - 企业内部应用可在[开发者后台](https://open-dev.dingtalk.io/#/appMgr/inner/h5/836390886/1)的应用详情页面查看。 - 第三方企业应用可调用[获取企业授权信息](/document/development/obtains-the-basic-information-of-an-enterprise#)接口获取。 |
