---
title: "更新群会话"
description: "调用本接口可通过群 chatid 更新指定群会话的基本信息及成员,包括群名称、群主、成员列表和群头像等。"
---
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| chatid | String | 是 | 群会话ID，可通过调用[创建群会话](/zh/open/development/create-common-group-new-version-v2)接口获取的chatid参数值。  不支持通过调用前端JSAPI获取的chatid。 |
| name | String | 否 | 群名称，长度限制为1~20个字符。 |
| owner | String | 否 | 群主的userId，可通过[根据手机号查询用户](/zh/open/development/query-users-by-phone-number)接口获取userId。  该员工必须为会话useridlist的成员之一。 |
...
| add_extidlist | Array of String | 否 | unionid列表，可通过[查询用户详情](/zh/open/development/query-users-by-phone-number)接口获取。 |
| del_extidlist | Array of String | 否 | unionId列表，可通过[查询用户详情](/zh/open/development/query-users-by-phone-number)接口获取。 |
| icon | String | 否 | 群头像的mediaId，可通过[上传媒体文件](/zh/open/development/upload-media-files)接口获取media_id参数值。 |
