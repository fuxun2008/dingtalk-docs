---
title: "创建条件格式规则"
description: "调用本接口可在钉钉表格的指定工作表中创建条件格式规则，根据单元格的值或重复情况自动应用高亮样式。适用于需要对数据进行可视化标记的场景，例如将低于目标值的单元格标红、将重复录入的数据高亮显示以便排查，或将某个区间内的数值以特定背景色区分。规则创建后，满足条件的单元格将自动应用指定的字体颜色或背景色，无需手动逐一设置。"
---
