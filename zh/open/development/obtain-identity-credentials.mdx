---
title: "获取登录用户的访问凭证"
description: "通过创建应用、OAuth 登录授权、获取访问凭证、调用 API 四个步骤,为应用代表登录用户调用钉钉 OpenAPI 获取访问凭证。"
---
...
使用返回的auth_code和应用的信息，调用[获取用户token](/zh/open/development/obtain-user-token)接口得到access_token。
...
获取access_token后，就可以使用这个凭证调用[获取用户通讯录个人信息](/zh/open/development/dingtalk-retrieve-user-information)接口。