...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| toCorpIds | Array of String | 是 | 被授权组织对应corpId列表。  被授权组织的corpId列表和调用接口的调用者对应 userId，都不能为空且参数出错会导致调用报错。 |
| optUserId | String | 是 | 当前调用接口的调用者对应userId。  用于数据审计，需填写当前组织下真实的员工userId，否则接口调用会失败。 |
| fields | Array of String | 否 | 授权字段:  如果fields为空或者空集合表示仅授权mobile（历史版本兼容）   - **mobile**：企业账号手机号。 - **status**： 企业账号停用状态。 |
