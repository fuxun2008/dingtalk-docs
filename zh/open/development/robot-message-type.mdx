...
| ActionCard类型 | sampleActionCard | ``` {   "title": "测试标题",   "text": "内容测试",   "singleTitle": "查看详情",   "singleURL": "https://open.dingtalk.io" } ``` | 卡片消息：一个按钮。一个按钮 |
...
| --- | --- | --- | --- |
| 20001 | 受调用量超量影响，当前我的消息服务已经暂停。请联系你所在的组织管理员并予以处理  接收消息中，新增 **errorMessage** 字段，用于展示错误信息。 | - 接收消息中无 text 和 content 字段内容 - 登录[开发者后台](https://open-dev.dingtalk.io/)即可查看调用量。  image | 需要升级钉钉专业版或购买增购包，详情参看[企业增购包购买说明](https://open.dingtalk.io/document/orgapp/purchase-instructions-for-enterprise-additional-purchase-package)。 |
