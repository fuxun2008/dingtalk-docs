...
- 本接口与**查询日程列表**接口的区别在于对循环日程的处理。本接口会根据循环规则展开查询区间内的所有循环日程，但不会实际在数据库中生成对应实例。 例如，有一个从1.1号开始的每日循环日程，查询时已经是1.4号，查询区间为1.1号至1.10号，那么本接口会返回1.1～1.10号的所有循环日程，而[查询日程列表](https://open.dingtalk.io/document/development/query-an-event-list#)接口是不会返回1.4号之后的日程。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| userId | String | 是 | 要查询的用户unionId。   - 企业内部应用和第三方企业应用，调用[查询用户详情](https://open.dingtalk.io/document/development/query-user-details#)接口获取unionid参数值。 - 第三方个人应用，调用[获取用户通讯录个人信息](https://open.dingtalk.io/document/development/dingtalk-retrieve-user-information#)接口获取unionId参数值。 |
| calendarId | String | 是 | 日程所属的日历ID，调用[查询日历](https://open.dingtalk.io/document/development/query-a-calendar#)接口查询当前用户日程，使用**primary**关键字，表示当前用户的主日历。 |
...
| timeMax | String | 否 | 查询日程开始时间 `< timeMax`，格式为 ISO-8601 的 date-time 格式。 |
