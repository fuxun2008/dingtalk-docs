---
title: "钉盘文件的上传、预览和下载"
description: "介绍通过文档与文件接口完成钉盘文件上传、预览和下载的完整开发流程,含前提条件、创建应用与网页应用配置步骤。"
---
...
2. 钉盘文件上传
   ...
   1. 调用[搜索文件](/zh/open/development/search-for-files)接口，搜索钉盘空间中的名为“测试文件夹”，并返回文件的 `dentryUuid`。

### 说明

      在调用该接口时，请先确保钉盘空间内只有一个名为“测试文件夹”的文件夹，否则可能会报错。
   2. 根据`dentryUuid`调用[获取文件上传信息](/zh/open/development/obtain-file-upload-informations)接口，获取文件的上传信息，包括`resourceUrls`和`headers`参数值。
   3. 使用OSS的header加签方式上传文件，如何上传，可参考[获取文件上传信息](/zh/open/development/obtain-file-upload-informations)接口中步骤 2。
   4. 调用[提交文件](/zh/open/development/submittal-file)接口，提交当前文件到钉盘，并获取返回的`spaceId`、`parentId`、`path`、`uuid`等参数值。
3. 钉盘文件预览

   1. 通过钉盘文件上传中最后提交时获取的 `uuid`，拼接并打开文件，进行预览。
4. 钉盘文件下载

   1. 通过调用[获取空间列表](/zh/open/development/queries-a-space-list)接口，获取钉盘空间信息，获取`spaceId`。
   2. 通过 spaceId 调用[获取文件或文件夹列表](/zh/open/development/get-a-list-of-files-or-folders)接口，获取文件的 `fileId` 和文件名称 `name`。
   3. 通过获取的 filedId 和 name [获取文件下载信息](/zh/open/development/obtains-the-download-information-about-a-file)接口，获取文件的下载信息，包括`internalResourceUrls` 和 `headers` 参数值。
   4. 通过以上的内容， 执行下载操作，下载代码请参考[获取文件下载信息](/zh/open/development/obtains-the-download-information-about-a-file)步骤 2。
...
2. 单击**应用开发** > **企业内部应用** > **钉钉应用** > **创建应用**。
   ...
   ![步骤一：创建应用](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3825544571/p994229.png)
3. 填写应用信息。
...
4. 单击**保存**，进入应用详情页。
   ...
   ![步骤一：创建应用](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3825544571/p994234.png)
...
1. 应用配置完成后，你需要发布应用，在应用详情页，单击**应用发布** > **版本管理与发布**。
   ...
   ![步骤四：发布应用](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3825544571/p994243.png)
2. 单击**创建新版本**，进入版本详情页面。
3. 配置版本信息：
...
4. 打开后端代码目录，在 resources 目录中修改`application.properties`文件，填写`clientId（应用Client ID）`和`clientSecret（应用Client Secret）`参数。
   ...
   ![步骤五：构建服务](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3825544571/p994245.png)
5. 点击启动后端服务。
...
3. 点开应用，点击**选择本地文件**，将本地文件上传到钉盘。
   ...
   ![步骤六：测试应用](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3825544571/p994249.png)

   选择文件后，单击**提交**即可，在文件信息确认中能看到用户的 ID 和文件名称及文件大小。

   ![步骤六：测试应用](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3825544571/p994250.png)
4. 上传成功后，如下图所示：
...
### 说明
   ...
   ![步骤六：测试应用](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3825544571/p994251.png)

   上传成功，可在钉盘的“测试文件夹”中查看已经上传成功的文件。

   ![步骤六：测试应用](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3825544571/p994252.png)
5. 通过点击文件下载按钮，完成文件的下载。

### 重要

   在文件下载时若出现“**auth level of org is not enough**”错误信息，则证明当前企业认证等级过低，需要升级后使用，详情请参考[获取文件下载信息](/zh/open/development/obtains-the-download-information-about-a-file)接口中的错误码说明。

   ![步骤六：测试应用](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3825544571/p994253.png)

   文件下载成功后，可在项目的目录中查看已经下载的文件。

   ![步骤六：测试应用](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3825544571/p994254.png)
