...
description: "调用本接口查询企业机器人在群聊发送消息后的发送状态及已读用户 userId 等信息。"
---
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| openConversationId | String | 否 | 群ID，可通过客户端 [chooseChat](https://open.dingtalk.com/document/development/jsapi-choose-chat#)获取群会话 openConversationId。  需要与[机器人发送群聊消息](/zh/open/development/the-robot-sends-a-group-message#)接口时使用的openConversationId一致。 |
| robotCode | String | 否 | 机器人的编码，详情参考[机器人 ID](https://open.dingtalk.com/document/development/robot-overview#)。  需要与[机器人发送群聊消息](/zh/open/development/the-robot-sends-a-group-message#)接口时使用的robotCode一致。 |
| processQueryKey | String | 是 | 消息唯一标识，通过[机器人发送群聊消息](/zh/open/development/the-robot-sends-a-group-message#)接口返回参数processQueryKey字段中获取。 |
