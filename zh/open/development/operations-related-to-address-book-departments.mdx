...
3. 获取应用访问凭证获取企业内部应用的access\_token。调用接口时，通过accessToken鉴权调用者身份。
4. 调用通讯录相关API：
   ...
   1. 调用服务端API-[创建部门](/zh/open/development/address-book-creation-department-established-department)接口，实现创建部门，获取部门`dept_id`**。**
      ...
      - 如果创建的是其他部门的子部门，需要先调用[获取部门列表](/zh/open/development/user-management-acquires-the-list-departments)接口，获取的部门ID作为parent\_id的值。
