...
description: "调用本接口查询指定用户所属的所有父级部门列表，不受通讯录权限范围限制。"
---
...
| request_id | String | nfe328zf67ec | 请求ID。 |
| errmsg | String | 不合法的userid | 调用失败时返回的错误信息。 |
| result | DeptListParentByUserResponse |  | 返回结果。 |
| parent_list | DeptParentResponse[] |  | 父部门列表集合。 |
| parent_dept_id_list | Number[] | [456, 123, 1] | 父部门列表。 |
