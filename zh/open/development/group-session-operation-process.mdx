...
description: "本文档介绍了如何调用群会话相关接口实现群会话操作的相关流程。首先创建一个企业内部应用，再使用群会话提供的API，实现创建群会话、获取群会话信息、群会话管理操作、修改群会话流程。"
---
...
1. 调用服务端API-[创建群](/document/development/session-management-creates-groups#)接口，进行创建群会话，获取群会话`chatId`。
...
3. 根据群会话chatId进行群会话管理操作。
   ...
   - 根据群会话`chatId`，调用新版服务端API-[获取群会话的OpenConversationId](/zh/open/development/obtain-group-openconversationid)接口，获取群会话`openConversationId`。
     ...
     - 根据群会话`openConversationId`，调用新版服务端API-[批量设置企业群管理员](/document/development/batch-setup-group#)接口，批量设置群管理员。
...
服务端API差异详情参见[旧版API VS 新版API](/document/development/how-to-call-apis#section-8lr-id4-rbz)。以下接口均使用新服务端API接口，SDK下载详情参见[服务端SDK下载](/document/development/download-the-server-side-sdk#)。
...
1. 调用服务端API-[创建群](/document/development/session-management-creates-groups#)接口，进行创建群会话，获取群会话chatId。
