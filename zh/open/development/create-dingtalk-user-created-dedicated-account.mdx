...
| login\_id | String | 是 | login\_id3 | 钉钉自建企业账号的登录名。  **说明**  建议不要携带手机号、邮箱链接等信息，避免注册短信被运营商管控拦截， |
| init\_password | String | 是 | init\_password220 | 钉钉自建企业账号的初始密码，初始密码至少8个字符。  **说明**   - 不能全是字母或者数字。 - 建议不要携带手机号、邮箱、链接等信息，避免注册短信被运营商管控拦截， |
