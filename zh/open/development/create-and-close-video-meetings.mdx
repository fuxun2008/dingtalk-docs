---
title: "创建、查询及关闭视频会议"
description: "本文档介绍了如何调用会议接口创建视频会议等流程。首先创建一个企业内部应用，再使用会议提供的API，实现创建视频会议、关闭视频会议和批量查询视频会议信息流程。"
---
...
1. 调用服务端API-[创建视频会议](/zh/open/development/create-a-video-conference)接口，进行创建视频会议，获取视频会议conferenceId。
2. 根据视频会议conferenceId，调用服务端API-[批量查询视频会议信息](/zh/open/development/batch-query-of-video-conference-information)接口，获取视频会议信息及参会人员列表。
3. 根据视频会议conferenceId，调用服务端API-[开启视频会议云录制](/zh/open/development/video-conference-open-cloud-recording)接口，开启视频会议云录制。
4. 调用视频会议conferenceId，调用服务端API-[开启视频会议云录制](/zh/open/development/video-conference-open-cloud-recording)接口，关闭视频会议云录制。
5. 根据视频会议conferenceId，调用服务端API-[关闭视频会议](/zh/open/development/close-audio-video-conference)接口，进行关闭视频会议。
6. 查询会议录制信息。

   1. 根据视频会议conferenceId，调用服务单API-[查询会议录制的详情信息](/zh/open/development/query-recording-information)接口，实现查询会议录制的录制人、录制开始时间、录制持续时长等信息。
   2. 根据视频会议conferenceId，调用服务端API-[查询会议录制中的视频信息](/zh/open/development/queries-the-playback-information-about-a-recorded-cloud-video)接口，实现查询视频下载地址、视频文件大小和视频时长等信息。
   3. 根据视频会议conferenceId，调用服务端API-[查询会议录制中的文本信息](/zh/open/development/queries-the-text-information-about-cloud-recording)接口，实现查询会议录制的文本内容、文本记录开始时间、文本记录结束时间等信息。
...
服务端API差异详情参见[旧版API VS 新版API](/document/development/how-to-call-apis#section-8lr-id4-rbz)。以下接口均使用服务端API接口，SDK下载详情参见[服务端SDK下载](/document/development/download-the-server-side-sdk#)。
...
3. 根据视频会议conferenceId，调用服务端API-[开启视频会议云录制](/zh/open/development/video-conference-open-cloud-recording)接口，开启视频会议云录制。
   ...
   public void cloudRecordsStart() throws Exception &#123;
       ...
       &#125;
   ```
4. 调用视频会议conferenceId，调用服务端API-[停止视频会议云录制](/zh/open/development/video-conferencing-stops-cloud-recording)接口，关闭视频会议云录制。
