...
- 如果上传文件时提供的入参为spaceId字段，使用[获取文件上传信息](https://open.dingtalk.io/document/development/obtain-storage-upload-information#)接口。
...
步骤三：调用[提交文件](/zh/open/development/submittal-file)接口，完成文件上传。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| parentDentryUuid | String | 是 | 父节点dentryUuid，可调用[搜索文件](https://open.dingtalk.io/document/development/search-for-files#)或[获取 dentryUuid 信息](https://open.dingtalk.io/document/development/api-getuuidbydentryid#)接口，获取返回参数`dentryUuid`字段。    如果是空间根目录, 填空间根目录的dentryUuid。 |
...
| name | String | 否 | 文件名称（用于文件名称合法性和文件名称冲突校验）。规则：- 头尾不能包含空格，否则会自动去除。 - 不能包含特殊字符，包括：制表符、`*`、`"`、`<`、`>`、`|`。 - 不能以 `.` 结尾。 |
...
| headerSignatureInfo | Object | Header加签上传信息。    当protocol参数传HEADER_SIGNATURE时，返回该字段。 |
