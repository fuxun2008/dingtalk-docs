...
- 日程参与者的添加和删除：建议使用[新增日程参与人](https://open.dingtalk.io/document/development/add-schedule-participant#)和[删除日程参与人](https://open.dingtalk.io/document/development/delete-schedule-participant#)接口。
...
| x-acs-dingtalk-access-token | String | 是 | 调用该接口的访问凭证，通过以下获取：   - 企业内部应用，调用[获取企业内部应用的accessToken](https://open.dingtalk.io/document/development/obtain-the-access-token-of-an-internal-app#)接口获取。 - 第三方企业应用，调用[获取企业accessToken(应用商店应用)](https://open.dingtalk.io/document/development/obtain-the-access-token-of-the-authorized-enterprise-1#)接口获取。 - 第三方个人应用，调用[获取用户token](https://open.dingtalk.io/document/development/obtain-user-token#)接口获取。 |
...
| id | String | 否 | 用户的unionId。   - 企业内部应用和第三方企业应用，调用[查询用户详情](https://open.dingtalk.io/document/development/query-user-details#)接口获取unionid参数值。 - 第三方个人应用，调用[获取用户通讯录个人信息](https://open.dingtalk.io/document/development/dingtalk-retrieve-user-information#)接口获取unionId参数值。 |
