---
title: "创建日程"
description: "调用本接口可在指定日历中创建一个新的日程，附接口调用说明、请求参数和响应字段。"
---
...
- 日程参与者的添加和删除：建议使用[新增日程参与人](/zh/open/development/add-schedule-participant#)和[删除日程参与人](/zh/open/development/delete-schedule-participant#)接口。
...
| x-acs-dingtalk-access-token | String | 是 | 调用该接口的访问凭证，通过以下获取：   - 企业内部应用，调用[获取企业内部应用的accessToken](/zh/open/development/obtain-the-access-token-of-an-internal-app#)接口获取。 - 第三方企业应用，调用[获取企业accessToken(应用商店应用)](https://open.dingtalk.com/document/development/obtain-the-access-token-of-the-authorized-enterprise-1#)接口获取。 - 第三方个人应用，调用[获取用户token](/zh/open/development/obtain-user-token#)接口获取。 |
...
| id | String | 否 | 用户的unionId。   - 企业内部应用和第三方企业应用，调用[查询用户详情](/zh/open/development/query-user-details#)接口获取unionid参数值。 - 第三方个人应用，调用[获取用户通讯录个人信息](/zh/open/development/dingtalk-retrieve-user-information#)接口获取unionId参数值。 |
