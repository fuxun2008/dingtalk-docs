---
title: "查询日程列表"
description: "调用本接口可查询某用户在指定时间段内的日程列表，可获取日程标题、开始日期、开始时间、结束日期和结束时间等信息。"
---
...
- 默认不会返回已删除的日程，如果需要返回可以设置`showDeleted=true`，当返回的是已删除的日程时，此时调用[查询单个日程详情](/zh/open/development/query-details-about-an-event#)接口会返回**instanceNotExist**的错误。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| userId | String | 是 | 查询目标用户的unionId。   - 企业内部应用和第三方企业应用，调用[查询用户详情](/zh/open/development/query-user-details#)接口获取unionid参数值。 - 第三方个人应用，调用[获取用户通讯录个人信息](/zh/open/development/dingtalk-retrieve-user-information#)接口获取unionId参数值。 |
...
| maxAttendees | Integer | 否 | 每个日程的参与者查询个数，默认100，最大100。 如果参会人数超过100人，需要拉取全部参会人请使用[获取日程参与者](/zh/open/development/get-the-participants-of-a-schedule#)接口获取。 |
