---
title: "查询日程列表"
description: "调用本接口，查询某用户在某段时间内的日程列表，可以获取到日程标题、日程的开始日期、开始时间、结束日期、结束时间等信息。"
---
...
- 默认不会返回已删除的日程，如果需要返回可以设置`showDeleted=true`，当返回的是已删除的日程时，此时调用[查询单个日程详情](https://open.dingtalk.com/document/development/query-details-about-an-event#)接口会返回**instanceNotExist**的错误。
...
| --- | --- | --- | --- |
| 名称 | 类型 | 是否必填 | 描述 |
| userId | String | 是 | 查询目标用户的unionId。   - 企业内部应用和第三方企业应用，调用[查询用户详情](https://open.dingtalk.com/document/development/query-user-details#)接口获取unionid参数值。 - 第三方个人应用，调用[获取用户通讯录个人信息](https://open.dingtalk.com/document/development/dingtalk-retrieve-user-information#)接口获取unionId参数值。 |
