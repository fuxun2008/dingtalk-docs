...
| groupId | Long | 否 | 会议室所属分组ID，可调用[查询会议室分组列表](https://open.dingtalk.io/document/development/query-meeting-rooms-groups)接口获取。 |
| reservationAuthority | Object | 否 | roomStatus部分权限时，设置的会议室预定权限。 |
| authorizedMembers | Array | 否 | 有权限的成员列表。 |
| memberId | String | 否 | 成员的id，memberType设置   - **人员**：成员unionId，可调用[查询用户详情](https://open.dingtalk.io/document/development/query-user-details)接口获取 - **部门**：部门id，dept\_id，可调用[获取部门列表](https://open.dingtalk.io/document/development/obtain-the-department-list-v2)接口获取dept\_id参数值。 |
