...
- 企业内部应用推荐直接通过开发者后台订阅回调事件，详情请参考配置事件订阅。
- 第三方企业应用推荐使用SyncHTTP推送和RDS推送方式订阅回调事件，这两种方式可直接使用推送的数据，配置更简单。详情请参考回调概述。
...
- 管理员开通第三方企业应用时，向应用推送企业授权开通应用事件，开发者可以根据推送的corpid进行企业激活和应用开通。
