---
title: "HTTP回调概述"
description: "开发者可以使用HTTP的方式注册钉钉的回调事件，用于接收钉钉推送的消息。例如企业授权开通应用事件、通讯录变更事件等，可以让开发者更好的与钉钉集成。"
---
...
- 企业内部应用推荐直接通过开发者后台订阅回调事件，详情请参考[配置事件订阅](https://open.dingtalk.com/document/app/configure-event-subcription#topic-2024355)。
- 第三方企业应用推荐使用SyncHTTP推送和RDS推送方式订阅回调事件，这两种方式可直接使用推送的数据，配置更简单。详情请参考[回调概述](https://open.dingtalk.com/document/app/callback-events-overview#topic-2024377)。
- HTTP推送的出口IP为203.119.0.0/16 140.205.0.0/16 106.11.0.0/16 198.11.0.0/16。

## 使用场景

- 在你的业务对数据的实时性要求较高时。例如：在新员工入职或者离职时，应用需要第一时间变更用户数据，此时就可以订阅[通讯录事件](https://open.dingtalk.com/document/app/tvgq1n#topic-1950634)。
- 你的应用需要及时响应用户的操作时。例如：某用户加入某群聊时，应用可以订阅[群会话事件](https://open.dingtalk.com/document/app/group-session-events#topic-1950637)，在用户进入群聊的时候，向用户发送欢迎等信息。
- 管理员开通第三方企业应用时，向应用推送[企业授权开通应用事件](https://open.dingtalk.com/document/app/enterprise-authorized-application-activation-event#topic-2021949)，开发者可以根据推送的corpid进行企业激活和应用开通。
...
2. 钉钉会向应用配置的HTTP地址推送回调事件数据包。使用应用创建时开发者在开发者后台填写的应用加解密key来进行加密，开发者填写的应用Token进行签名。详情请参考[测试回调URL事件](https://open.dingtalk.com/document/app/test-callback-url#topic-2001893)。
3. 应用在收到推送后需要进行验证签名和解密，并且返回包含经过加密的字符串的JSON数据。
4. 调用[注册回调事件](https://open.dingtalk.com/document/app/registers-event-callback-interfaces#topic-1936765)接口接收回调事件。
...
2. 单击**开发管理**，然后单击**修改**，推送类型选择**HTTP推****送**。
3. 配置用于接收请求的HTTP地址。

   - **token**：钉钉每次向你的地址推送事件数据时都会携带`token`，用于生成签名、校验回调请求的合法性。必须为英文或数字，长度为3~32个字符。
   - **数据加密密钥**：单击**自动生成**生成AES密钥。回调消息内容的加解密参数，是AES密钥的Base64编码，具体使用参见[消息加解密](#section-zis-gsf-dlt)。
...
4. 配置完成后，单击**验证有效性**按钮时，开放平台会向你配置的网址推送一个application/json格式的 POST请求, 用于验证你配置的网址的合法性。请求如下：
   ...
   ![成功](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1108778061/p205612.png)
...
- [事件列表](https://open.dingtalk.com/document/app/test-callback-url-events-1#topic-2001893)
