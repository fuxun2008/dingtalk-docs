---
title: "概述"
description: "在调用 DingTalk OpenAPI 之前，应用必须从钉钉开放平台获取访问凭证（accessToken）。该凭证包含应用的身份信息及其对钉钉资源的访问权限，是调用 API 的必要认证凭据。"
---
...
| --- | --- | --- |
| 授权凭证 | 应用场景 | 获取方式 |
| 用户accessToken | 需要登录用户授权的应用。 | [获取用户token](/zh/open/development/obtain-user-token) |
| 企业内部应用accessToken | 企业内部应用调用服务端API获取应用资源。 | [获取企业内部应用的accessToken](/zh/open/development/obtain-the-access-token-of-an-internal-app) |
...
- 获取登录用户的访问凭证：参考[获取登录用户的访问凭证](/zh/open/development/obtain-identity-credentials)文档教程。
- 获取无登录用户的访问凭证：参考[获取应用的API访问凭证](/zh/open/development/used-application-authorization)文档教程。
