...
6. 在开发者后台-场景群页面，创建[群模板](https://open-dev.dingtalk.io/fe/im#/group/list)，获取群模板id参数**template_id**。
...
8. 进入对应群模板，提交审核，点击发布。
9. 依据**template_id**调用创建群接口，获取**openConversationId**。
...
9. 通过**template_id**调用创建群接口，获取**openConversationId**。
10. 调用发送群助手消息接口，实现机器人群消息的发送。