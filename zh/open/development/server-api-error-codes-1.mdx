---
title: "全局错误码"
description: "在调用钉钉开放平台接口时，开发者会收到对应的返回结果，包含成功或失败的错误码信息。通过分析错误码，可快速定位问题并进行调试。"
---
...
| 403 | Forbidden.AccessDenied.AccessTokenPermissionDenied | 没有调用该接口的权限 | 需要申请下接口权限，请参考[添加接口调用权限](https://open.dingtalk.com/document/development/add-api-permission)。 |
...
| 33012 | 无效的USERID | 无效的USERID。请检查userid是否正确，可通过[获取部门用户userid列表](https://open.dingtalk.com/document/development/query-the-list-of-department-userids)接口获取。 |  |
...
| 34004 | 无效的会话消息的类型 | 无效的会话消息的类型。检查下msgtype字段，是否为空，是否是定义的类型。msgtype请参考[消息通知类型](https://open.dingtalk.com/document/development/message-types-and-data-format)。 |  |
| 34006 | 发送者不在企业中 | 发送者不在企业中。检查下发送者是否在企业中。可通过[获取部门用户基础信息](https://open.dingtalk.com/document/development/queries-the-simple-information-of-a-department-user)查看。 |  |
...
| 34010 | 文件不能为空 | 文件不能为空。检查下media\_id字段是否为空。可通过[上传媒体文件](https://open.dingtalk.com/document/development/upload-media-files)接口获取。 |  |
| 34011 | 音频文件不能为空 | 音频文件不能为空。检查下media\_id字段是否为空。可通过[上传媒体文件](https://open.dingtalk.com/document/development/upload-media-files)接口获取。 |  |
| 34012 | 找不到发送者的企业 | 找不到发送者的企业。检查下发送者是否是真实的。可通过[获取部门用户userid列表](https://open.dingtalk.com/document/development/query-the-list-of-department-userids)接口获取。 |  |
| 34013 | 找不到群会话对象 | 找不到群会话对象。检查下chatid是否真实存在。可通过[查询群信息](https://open.dingtalk.com/document/development/obtain-a-group-session)接口查看。 |  |
...
| 40003 | 不合法的UserID | 不合法的UserID。确保该id在通讯录中存在，且是在你所传access\_token对应的企业里。可通过[获取部门用户userid列表](https://open.dingtalk.com/document/development/query-the-list-of-department-userids)接口获取。 |  |
...
| 40007 | 不合法的媒体文件id | 不合法的媒体文件id。检查下mediaId是否为空，是否真实存在。可通过[上传媒体文件](https://open.dingtalk.com/document/development/upload-media-files)接口获取。 |  |
| 40008 | 不合法的消息类型 | 不合法的消息类型。检查下msgtype是否为空，确保它在开放平台定义的几种类型里，msgtype请参考[消息通知类型](https://open.dingtalk.com/document/development/message-types-and-data-format)。 |  |
| 40009 | 不合法的部门id | 不合法的部门id。检查下部门id是否为空，是否为数字且大于0。可通过[获取部门列表](https://open.dingtalk.com/document/development/user-management-acquires-the-list-departments)接口查看。 |  |
| 40010 | 不合法的父部门id | 不合法的父部门id。检查父部门id是否为一个数字。可通过[获取指定用户的所有父部门列表](https://open.dingtalk.com/document/development/queries-the-list-of-all-parent-departments-of-a-user)或[获取指定部门的所有父部门列表](https://open.dingtalk.com/document/development/query-the-list-of-all-parent-departments-of-a-department)接口获取。 |  |
...
| 41006 | 缺少media\_id参数 | 缺少media\_id参数。检查下media\_id参数是否为空。可通过[上传媒体文件](https://open.dingtalk.com/document/development/upload-media-files)接口获取。 |  |
...
| 41012 | 缺少应用头像mediaid | 缺少应用头像mediaid。检查下mediaid是否为空。可通过[上传媒体文件](https://open.dingtalk.com/document/development/upload-media-files)接口获取。 |  |
...
| 44001 | 多媒体文件为空 | 多媒体文件为空。请检查多媒体文件数据是否为空。可通过[上传媒体文件](https://open.dingtalk.com/document/development/upload-media-files)接口获取。 |  |
...
| 49017 | 查询企业外部联系人不存在 | 查询企业外部联系人不存在。可通过[获取外部联系人列表](https://open.dingtalk.com/document/development/obtain-the-external-contact-list)接口获取。 |  |
| 49018 | 群主非企业员工 | 群主非企业员工。无。 |  |
| 49019 | 群主非企业外部通讯录人员 | 群主非企业外部通讯录人员。可通过[获取外部联系人列表](https://open.dingtalk.com/document/development/obtain-the-external-contact-list)接口获取。 |  |
...
| 60003 | 部门不存在 | 部门不存在。可通过[获取部门列表](https://open.dingtalk.com/document/development/user-management-acquires-the-list-departments)接口获取。 |  |
| 60004 | 父亲部门不存在 | 父亲部门不存在。可通过[获取指定用户的所有父部门列表](https://open.dingtalk.com/document/development/queries-the-list-of-all-parent-departments-of-a-user)接口获取。 |  |
| 60005 | 不允许删除有成员的部门 | 不允许删除有成员的部门。可通过[获取部门用户userid列表](https://open.dingtalk.com/document/development/query-the-list-of-department-userids)接口查看。 |  |
| 60006 | 不允许删除有子部门的部门 | 不允许删除有子部门的部门。可通过[获取子部门ID列表](https://open.dingtalk.com/document/development/obtain-the-list-of-sub-department-ids)接口查看。 |  |
...
| 60016 | 部门id已经存在 | 部门id已经存在。可通过[获取部门详情](https://open.dingtalk.com/document/development/query-department-details0-v2)接口查看详情。 |  |
| 60017 | 不允许设置企业 | 不允许设置企业。无。 |  |
| 60018 | 不允许更新根部门 | 不允许更新根部门。无。 |  |
| 60019 | 从部门查询人员失败 | 从部门查询人员失败。请检查该成员是否在该部门中。可通过[获取部门用户userid列表](https://open.dingtalk.com/document/development/query-the-list-of-department-userids)接口获取。 |  |
| 60020 | 访问ip不在白名单之中 | 访问ip不在白名单之中。企业应用：检查配置的服务器出口ip地址是否和请求ip地址一致。 isv应用：检查套件ip白名单和请求ip是否一致。 |  |
| 60021 | 父部门下该部门名称已存在 | 父部门下该部门名称已存在。可通过[获取部门列表](https://open.dingtalk.com/document/development/user-management-acquires-the-list-departments)接口查看。 |  |
