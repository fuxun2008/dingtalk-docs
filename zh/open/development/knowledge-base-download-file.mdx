...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| spaceId | String | 是 | 空间Id，可通过[获取节点列表](/zh/open/development/get-node-list)接口获取知识库节点的 `dentryUuid`，再调用[根据 dentryUuid 获取 spaceId](/zh/open/development/api-getdentryidbyuuid)接口获取 `spaceId`。 |
| dentryId | String | 是 | 文件Id，可通过[获取节点列表](/zh/open/development/get-node-list)接口获取知识库节点的 `dentryUuid`，再调用[根据 dentryUuid 获取 spaceId](/zh/open/development/api-getdentryidbyuuid)接口获取 `dentryId`。 |
...
| version | Long | 否 | 文件的版本号，可调用[获取文件版本列表](/zh/open/development/obtains-a-list-of-file-versions)接口获取version。 |
...
| headerSignatureInfo | Object | Header加签信息。  **说明**  当protocol字段值为HEADER_SIGNATURE时，此字段生效。 |
