---
title: "API 调用步骤详解"
description: "本指南旨在指导开发者从零开始，通过调用钉钉开放平台接口，完成从注册应用到接口调通的全流程操作。文档涵盖关键步骤、代码示例、注意事项及调试方法，确保开发者快速上手并成功调用API。"
---
...
1. **基础概念**：已了解钉钉开放平台的基础概念及各产品块文档说明，详见[基础概念](/document/development/development-basic-concepts#)文档说明。
2. **接口频率限制：**已了解API的调用频率限制，详见[调用频次与限流](/document/development/how-to-process-api-throttling-on-the-dingtalk-server#)文档说明。
...
4. **获取用户**`UserId`：登录[钉钉管理后台](http://oa.dingtalk.io/)并在**通讯录 > 成员管理**下查看`UserId`，详见[基础概念](/document/development/development-basic-concepts#)说明。
...
4. 保存应用的**Client ID**和**Client Secret**，用于后续接口调用使用。
   ...
   **Client ID**和**Client Secret**获取可参考[基础概念](/document/development/development-basic-concepts#)中说明，获取后请妥善保管，避免泄露。

## **步骤二：配置权限**

1. **查找所需权限**：根据接口文档，调用用户详情需`成员信息读权限`权限。

   ![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7516308771/p1071949.png)
...
2. 在调试工具中，替换`appkey（参数值替换为提前获取的Client ID）`和`appSecret（参数值替换为提前获取的Client Secret）`的值，并点击**发起调试**按钮。
   ...
   **重要** 
   ...
   ![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7516308771/p1071954.png)
...
申请敏感权限后，需要接入[授权套件](/document/development/function-description#)，用户在授权弹窗中同意授权时，返回`authCode`，然后调用[获取用户token](/zh/open/development/obtain-user-token)接口，换取用户级`access_token`。
...
若使用钉钉SDK进行调用，需要提前了解服务端 API 的差异，然后下载对应版本的SDK，详见[服务端 API 差异详解](/document/development/how-to-call-apis#)文档介绍。
