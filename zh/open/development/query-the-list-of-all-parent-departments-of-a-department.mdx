...
description: "调用本接口可获取指定部门的所有父部门 ID 列表，该接口不受通讯录权限范围限制。"
---
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| dept_id | Number | 是 | 42xxxx36 | 要查询的部门ID，可调用[获取部门列表](/zh/open/development/user-management-acquires-the-list-departments)接口获取dept_id参数值。 |
...
| 名称 | 类型 | 示例值 | 描述 |
| --- | --- | --- | --- |
| request_id | String | 10u9dj0853fwe | 请求ID。 |
| errcode | Number | 0 | 返回码。 |
| errmsg | String | 不合法的部门id | 调用失败时返回的错误信息。 |
| result | DeptListParentByDeptIdResponse |  | 父部门ID列表。 |
| parent_id_list | Number[] | [789, 456, 123, 1] | 该部门的所有父部门ID列表。  **说明**  获取的list元素第一个是当前部门ID，最后一个是父部门ID，从左至右部门层级递增。 |
