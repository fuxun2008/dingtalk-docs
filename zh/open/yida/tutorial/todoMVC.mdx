---
title: "TodoMVC"
description: "宜搭高级实战教程：参考 TodoMVC 从零搭建任务清单页面，综合运用全局变量、循环渲染、事件处理等自定义页面核心能力。"
---
...
<Frame>
  ![](https://img.alicdn.com/imgextra/i4/O1CN015YAJBaz0JoK7HAG4_!!6000000003442-1-tps-3584-1780.gif)
</Frame>
...
本文不详细介绍在宜搭中创建并部署自定义页面的具体流程，如果对创建流程不了解，请移步至 [快速开始](/zh/open/yida/guide/start) 文档，下面让我们一步一步来实现上面的效果吧。
...
根据 TodoMVC 的 UI 展示，我们可以找到宜搭中对应可以使用的组件（各组件的详细使用可以查看 [组件文档](/zh/open/yida/components/layout/tabsLayout)），如下所示：
...
<Frame>
  ![](https://img.alicdn.com/imgextra/i1/O1CN01b0O3g6fkTTH7HAG4_!!6000000005249-1-tps-3584-1780.gif)
</Frame>

### 步骤 1：创建页面全局变量

宜搭的设计理念和 React 类似都是 MVVM 模式，因此我们在进行功能开发前需要先定义页面中需要使用的数据模型，即全局状态（具体使用详见 [页面状态文档](/zh/open/yida/guide/concept/state)），我们定义以下全局变量：
...
现在我们就可以开始实现第一个功能了，在这一步我们希望用户再输入框中输入任务内容，并点击回车键创建一条待办任务（事件绑定使用详见 [事件处理文档](/zh/open/yida/guide/concept/event)）。
...
通过上一步，我们已经可以将用户输入的内容生成待办任务并添加到全局变量 todoList 中，接下来我们就需要在输入框的下方展示待办列表了，这一步会用到 [循环渲染](/zh/open/yida/guide/concept/loop) 的相关知识。
...
我们决定用 3 个组件（2 个文本组件+1 个输入框组件）来实现不同展示效果，并通过 [条件渲染](/zh/open/yida/guide/concept/condition) 绑定是否渲染属性来实现展示切换：
...
上一步我们完成的 TodoMVC 的基础功能实现，但是给人的第一感觉就是丑，对于一个产品好看的皮囊不是最重要的，但是如果没有一副好的皮囊，它一定不是一个好的产品，宜搭提供了自定义样式能力，详见 [样式定制文档](/zh/open/yida/guide/concept/style)，通过定制样式，我们将得到下效果，（用户也可以通过访问 [示例页面](https://www.aliwork.com/o/demo/todoMVC-3) 进行试用，通过 [设计器](https://www.aliwork.com/developer/designer?formUuid=todoMVC-1) 查看具体实现）：
...
<Frame>
  ![](https://img.alicdn.com/imgextra/i1/O1CN01RNUXROGCVND7HAG4_!!6000000008043-1-tps-3584-1780.gif)
</Frame>
...
首先，我们在当前应用下创建一个普通表单用于存储待办任务的信息，具体创建普通表单的方案详见 [宜搭使用文档](/zh/yida/form/ybuoxl)，这里不展开描述，如下所示：
...
有了存储任务数据的地方，我们就需要在 TodoMVC 中创建异步 API 用户实现任务的增删改查，宜搭设计器提供了远程 API 配置用于请求远程 http 接口（详见 [远程 API 文档](/zh/open/yida/guide/concept/datasource)），同时宜搭也提供了常用的 OpenAPI 接口来实现数据的基本操作（详见 [OpenAPI 文档](/zh/open/yida/api/openAPI)），我们在搭建页面中创建以下远程 API 配置：
<Frame>
