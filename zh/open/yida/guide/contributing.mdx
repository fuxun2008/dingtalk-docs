---
title: "贡献指南"
description: "介绍参与宜搭开发者中心文档共建的方式与流程，包括提交 Issues 反馈问题和通过 Pull Request 编辑文档的具体步骤。"
---
...
启动后，我们便可以在本地通过 `http://localhost:3000/` 进行访问。
...
必须在完成 `npm run build`之后，该命令会在本地基于build目录启动一个静态服务，通过 `http://localhost:3000/` 进行访问，效果和线上效果完全一致并支持全局搜索功能。
