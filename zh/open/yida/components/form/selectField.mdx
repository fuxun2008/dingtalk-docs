...
description: "SelectField 下拉选择组件文档，介绍选项数据源、搜索与联动等属性，在限定选项中完成单项选择。"
---
...
<iframe src="https://www.aliwork.com/developer/select-field-v2?isRenderNav=false" width="100%" height="480" frameBorder="0"></iframe>
...
| `filterLocal` | 支持本地过滤，在数据源为远程的时候需要关闭此项 | `boolean` | - |
| `hasArrow` | 是否有下拉箭头 | `boolean` | `true` |
| `hasBorder` | 是否有边框 | `boolean` | `true` |
| `hasClear` | 是否有清除按钮 | `boolean` | `true` |
| `label` | 表单组件标题 | `string` | - |
...
| `notFoundContent` | 为空文案 | `string` | `'请输入'` |
| `onChange` | 组件值发生改变事件 | `(value: string) => void` | - |
| `onRemove` | 值删除时事件 | `(item: object) => void` | - |
