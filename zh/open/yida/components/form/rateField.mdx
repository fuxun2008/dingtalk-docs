---
title: "RateField 评分"
description: "RateField 评分组件参考，介绍评分总数与展示样式等属性配置，常用于满意度调查等用户反馈场景。"
---
...
<iframe src="https://www.aliwork.com/developer/rate-field-v2?isRenderNav=false" width="100%" height="480" frameBorder="0"></iframe>

## 组件属性

| 属性 | 说明 | 类型 | 默认值 |
|---|---|---|---|
| `allowHalf` | 支持半星评分 | `boolean` | `false` |
...
| `onChange` | 用户点击评分时 | `({ value: number }) => void` | - |
| `onHoverChange` | 用户hover评分时 | `(value: number) => void` | - |
| `showGrade` | 显示分数 | `boolean` | `false` |
