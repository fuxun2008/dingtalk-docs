---
title: "成员资料页扩展互动卡片能力"
description: "本文介绍如何在成员资料页中通过酷应用实现互动卡片的数据响应与动态交互功能。通过该能力，开发者可在用户查看成员资料时，动态渲染自定义业务数据卡片，并支持用户点击交互后的回调处理。"
---
...
![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7345378661/p521278.png)
...
| corpId | String | 所属企业ID，从不同的入口进入成员资料页，企业ID不同。   - **内部群&外部群**查看成员资料页：返回该内部群&外部群所属的企业ID。image - **单聊&普通群**查看成员资料页：返回被查看人的**主企业ID**。image  **说明**  个人主企业可以从以下路径查看 **移动端 > 左上角个人头像 > 点击个人头像 >企业/团队。** |
| content | String | profile操作上下文，包含：   - dynamicParams.profile    - corpId: 被查看人企业ID   - userId：被查看人所在企业的用户userId - cardPrivateData（卡片数据回传时才会有）    - actionIds：点击选中的前端控件ID，示例如下图：image   - params：数据回传具体参数，示例如下图：image |
