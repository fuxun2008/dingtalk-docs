...
description: "Cross-spreadsheet references rely on the IMPORTRANGE function to import cell data from a target range in a specified spreadsheet and automatically sync updates from the source spreadsheet."
---
...
For example: =IMPORTRANGE("[https://alidocs.dingtalk.com/](https://docs.dingtalk.io/)","'Project Management'!G21:G22")
...
After confirming the reference, an authorization prompt appears in a small window. Click **Allow**, otherwise the reference cannot take effect. **See the example on the right** 👉
...
[Priority: 2] **Categories and causes of cross-spreadsheet reference failures**
...
[Priority: 3] **Notes for using cross-spreadsheet references**
...
3.  The referenced range cannot contain other data. Cross-spreadsheet references rely on "array formulas". When the array expands, the target area must be empty, as shown below.
