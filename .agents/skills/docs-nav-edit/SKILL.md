...
name: docs-nav-edit
version: 1.0.0
description: "Atomic, SearchReplace-only safe editor for the strictly order-synced trilingual docs.json navigation. Operations: add-product, add-group, add-page, reorder, verify. Use for any navigation structure change; never overwrite docs.json wholesale."
description_zh: "docs.json 三语 navigation 安全编辑器：add-product / add-group / add-page / reorder / verify 原子操作，强制精确替换，禁止整份覆盖。"
user-invocable: true
argument-hint: "<add-product|add-group|add-page|reorder|verify> [args]"
---
# docs.json 三语 navigation 安全编辑器

> 子 skill：被 [[docs-add-page]] 调用做原子操作（`add-page`）；也作为新增 product / group / 三语注册的入口

把 `docs.json`（2612 行单文件 + 三语 navigation 严格同序）的编辑从"手工 SearchReplace 易破坏顺序 / 误删兄弟 product"压成原子化命令。**强制只用 SearchReplace 工具精确字符串替换，绝对禁止 Write 整份覆盖**（历史踩坑：Write 一次毁掉作者风格 + 误删 2 个产品 tab）。
...
3. **建三语 index.mdx**（frontmatter only）：
   ...
   ```
4. **SearchReplace `docs.json` 三处**：在三语 `tabs[]` **末尾追加**（不要插中间）新 tab 对象：
   ```jsonc
...
3. **SearchReplace `docs.json` 三处**：在三语对应 tab 的 `groups[]` **末尾追加**（除非 `--after <existing-group>` 指定插入位置）新 group 对象：
   ```jsonc
...
**最严重历史踩坑**：用 Write 整份覆盖一次，毁掉作者风格 + 误删 2 个产品 tab，回滚后才发现。**本 skill 强制只用 SearchReplace 工具精确字符串替换**，每个 language 块单独 SearchReplace，三个 SearchReplace 调用串联。
