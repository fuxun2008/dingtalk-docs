---
name: docs-translate-polish
version: 1.0.0
description: "Polish existing en/ja translations for language quality only — terminology consistency, sentence length, active voice, punctuation, list parallelism, tense — while keeping semantics, links, components and frontmatter unchanged."
description_zh: "翻译润色（en / ja 已译文件二次打磨）：只改语言质量（术语一致性 / 句长 / 主动语态 / 标点），绝不改动语义。"
user-invocable: true
argument-hint: "<root> [--lang en|ja] [--limit N]"
---
# 翻译润色（en / ja 已译文件二次打磨）
