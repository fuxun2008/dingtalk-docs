---
name: docs-import-hub-en
version: 1.0.0
description: "EN-direct import route: import a DingTalk English hub into the help center in one pass (hub crawl, download, template-mirrored import, register the en tab) and skip translation. Use when the English master already exists upstream."
description_zh: "EN-direct 导入路线 — 钉钉文档英文 hub 一键导入帮助中心子产品（hub crawl → download → 仿模板 import → 注册 en tab，跳过翻译）。"
user-invocable: true
argument-hint: "<hub-url> <slug>"
---
...
### 8. docs.json 注册 en tab（SearchReplace 工具精确插入）
...
**用 SearchReplace 工具**精确字符串替换，**绝不 Write 覆盖整个 docs.json**（项目硬规则）。锚点形态：
...
- ❌ `Write` 覆盖整个 docs.json：用 `SearchReplace` 工具精确插入
