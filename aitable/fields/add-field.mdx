...
| **Standard fields** | |
| Text | Plain text |
| Single select / Multi-select | Tag records — one tag per cell (single) or multiple per cell (multi). To reuse the same option set across tables, build a master table of options and reference them via [single-select / multi-select option sync](/aitable/fields/field-types/sync-select-options). |
