...
<AccordionGroup>
<Accordion id="q1" title="After writing a formula, the cell shows #REF! or similar — what's wrong?">
...
<Accordion id="q2" title="Why might the formula be invalid?">
...
<Accordion id="q3" title={`How do you write "not equal" in an AI Table formula?`}>

  **`!=`** means "not equal" in AI Table formula fields.

</Accordion>
<Accordion id="q4" title="How do you express null in an AI Table formula?">

  Use `= ""` directly, or use `ISBLANK()` to test for empty.

</Accordion>
<Accordion id="q5" title="How do you input emoji or symbols in an AI Table formula?">
...
<Accordion id="q6" title="Can a formula field be converted to another field type?">
...
<Accordion id="q7" title="How do I convert a formula field to another type?">
...
<Accordion id="q8" title="SUM in a formula field returns empty or errors — what happened?">

  Likely the field type doesn't support sum. Text fields can't sum. Convert to number first.

</Accordion>
<Accordion id="q9" title="How do I adjust the format of a formula field's return value?">

  After computation, right-click the column header → Edit field/column → pick a number format.

</Accordion>
<Accordion id="q10" title="How do I change the display format of a formula field?">
...
<Accordion id="q11" title="TEXT() output errors when used in computation — what to do?">

  `TEXT()` returns text. Wrap with `SUM()`, `ABS()`, etc., to convert to a number first.

</Accordion>
<Accordion id="q12" title="SUM result is wrong — why?">
...
<Accordion id="q13" title="What's the data format of date function results?">
...
<Accordion id="q14" title="Date shows as a number when concatenated to text — why?">
...
<Accordion id="q15" title="Formula result should be a date but shows as a number — why?">

  Double-click the column header, set format to date.

</Accordion>
<Accordion id="q16" title={`How do you express "null" in a formula?`}>
...
<Accordion id="q17" title="Why does IFS() return `#N/A`?">
...
<Accordion id="q18" title="How do I compute a date N working days later?">
...
<Accordion id="q19" title="Max fields a formula can reference?">

  300 fields per formula field.

</Accordion>
<Accordion id="q20" title="How to do basic per-row math?">
...
<AccordionGroup>
<Accordion id="q21" title="A field looks like a number but I can't do > or < on it — why?">
