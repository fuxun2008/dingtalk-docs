...
> ⚠️ 这 3 个脚本目前 hardcode 扫 `zh/docs/**`。**给新子产品复用时，先用 `Edit` 把脚本里的 `zh/docs/` 改为 `zh/<slug>/`**，跑完再改回——或者复制一份脚本为 `*_<slug>.py`。改完不要提交脚本本身，只提交产物。
...
逐个用 `Edit` 处理，参考既有产物 commit `66bc60e` 的 sed 模式：
...
9a. `docs.json` 三语 navigation 同步（**绝不用 Write 覆盖**，必须 Edit 精确插入）：
...
`docs.json` 必须用 `Edit` 工具精确插入。一次性 Write 覆盖会破坏作者风格 + 误删其他 4-12 个产品 tab。三语块严格同序：product slug 全英文不译，tab 显示名按语言翻译（中文 `文档` / 日文 `ドキュメント`），group 标题各语言自然翻译。
